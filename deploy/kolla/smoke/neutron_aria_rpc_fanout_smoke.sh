...
wait_for_initial_convergence() {
    ...
    while [ "${waited}" -lt "${STARTUP_CONVERGENCE_TIMEOUT}" ]; do
        if grep -q "full_resync_complete" "${log}"; then
            echo "initial_full_resync_converged_${label}=true waited=${waited}"
            return 0
        fi
        sleep 1
        waited=$((waited + 1))
    ...
    tail -120 "${log}" || true
    die "${label} initial full-resync did not converge within ${STARTUP_CONVERGENCE_TIMEOUT}s"
}
...
run_agent_case() {
    ...
    wait_for_initial_convergence "${log}" "${label}"
