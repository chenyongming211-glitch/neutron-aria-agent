...
write_temp_config() {
    local cfg="/tmp/neutron-aria-rpc-foreign-host.ini"
    local state_dir="/tmp/neutron-aria-rpc-foreign-host-state"
...
first_bound_port_id() {
    local host="$1"
    neutron_cli port-list -- --binding:host_id="${host}" |
...
print("rpc_foreign_port_update_sent port_id=%s binding_host=%s" % (
    port_id,
    binding_host,
))
PY
...
run_foreign_case() {
    local cfg log trigger_log rc initial_managed final_managed full_resync_count
    ...
    if [ -z "${EVENT_PORT_ID}" ]; then
        EVENT_PORT_ID="$(first_bound_port_id "${EVENT_BINDING_HOST}")"
    fi
    [ -n "${EVENT_PORT_ID}" ] || die "no port bound to ${EVENT_BINDING_HOST}"

    cfg="$(write_temp_config)"
    log="${WORK_DIR}/agent.log"
    trigger_log="${WORK_DIR}/trigger.log"

    echo "host=${HOST_FQDN} foreign_host=${EVENT_BINDING_HOST} foreign_port=${EVENT_PORT_ID}"
    ...
    grep -q "event_batch_drained" "${log}" || \
        die "foreign-host event did not reach the event merger"
    grep -q "port_updates=1" "${log}" || \
        die "foreign-host event did not include one port update"
    grep -q "service_result action=event_batch" "${log}" || \
        die "foreign-host event was not processed by the service loop"
    ...
    if [ "${full_resync_count}" != "1" ]; then
        die "foreign-host event triggered an unexpected full resync"
    fi

    assert_not_managed "${EVENT_PORT_ID}" | tee "${WORK_DIR}/foreign-port-managed-check.log"
    final_managed="$(managed_port_count)"
    echo "final_managed_ports_before_rollback=${final_managed}"
    if [ "${initial_managed}" != "${final_managed}" ]; then
        die "foreign-host event changed local managed port count"
    fi
}
...
echo "rpc_foreign_host_filter=pass work=${WORK_DIR}"
