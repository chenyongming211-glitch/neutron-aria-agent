...
SMOKE_CONFIG="${SMOKE_CONFIG:-/tmp/neutron-aria-agent-tap-recreate.ini}"
EXEC_USER="${EXEC_USER:-neutron}"
ROLLBACK="${ROLLBACK:-true}"
ALLOW_VM_REBOOT="${ALLOW_VM_REBOOT:-false}"
REBOOT_TYPE="${REBOOT_TYPE:-hard}"
...
cleanup() {
    if [ "${ROLLBACK_ARMED:-false}" = "true" ] && [ "${ROLLBACK}" = "true" ]; then
        echo "Rolling back tap recreate smoke managed ports"
...
wait_server_active() {
    ...
    die "server ${SERVER_ID} did not return to ACTIVE"
}
...
wait_port_on_host() {
    ...
    die "port ${EXPECTED_PORT_ID} did not bind back to ${HOST_FQDN}"
}
...
[ "${port_host}" = "${HOST_FQDN}" ] || \
    die "port ${EXPECTED_PORT_ID} is bound to ${port_host}, not ${HOST_FQDN}"

echo "Cleaning existing managed ports before tap recreate smoke"
rollback_managed_ports

echo "Applying baseline full-resync for ${EXPECTED_PORT_ID}"
run_agent_once
ROLLBACK_ARMED=true
wait_tap
before_ifindex="$(ifindex_of)"
assert_target_managed "${before_ifindex}"
assert_xdp_attached
wait_ping

if [ "${ALLOW_VM_REBOOT}" != "true" ]; then
    die "set ALLOW_VM_REBOOT=true to run ${REBOOT_TYPE} reboot of ${SERVER_ID}"
fi

echo "Rebooting server ${SERVER_ID} with nova reboot --${REBOOT_TYPE}"
openstack_cli "nova reboot --${REBOOT_TYPE} '${SERVER_ID}'"
wait_server_active
wait_port_on_host
wait_tap
after_ifindex="$(ifindex_of)"
echo "tap_ifindex_before=${before_ifindex} tap_ifindex_after=${after_ifindex}"
if [ "${REQUIRE_IFINDEX_CHANGE}" = "true" ] && [ "${before_ifindex}" = "${after_ifindex}" ]; then
    die "tap ifindex did not change; reboot did not recreate ${EXPECTED_IFNAME}"
fi
wait_ping

echo "Running full-resync after tap lifecycle event"
run_agent_once
current_ifindex="$(ifindex_of)"
assert_target_managed "${current_ifindex}"
assert_xdp_attached
wait_ping
...
echo "neutron-aria-agent tap recreate smoke passed for ${EXPECTED_PORT_ID}"
