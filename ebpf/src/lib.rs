...
pub fn xdp_firewall(ctx: XdpContext) -> u32 { unsafe {
    ...
    if info.is_ipv6 {
        return xdp_ingress_v6(&info, pkt_len, qos_on, tcprt_on, tracing, now);
    }
...
#[inline(never)]
unsafe fn xdp_ingress_v6(info: &parser::PacketInfo, pkt_len: u32, qos_on: bool, tcprt_on: bool, tracing: bool, now: u64) -> u32 {
    let ct_key = CtKey6 {
...
pub fn tc_egress(ctx: TcContext) -> i32 { unsafe {
    ...
    if info.is_ipv6 {
        return tc_egress_v6(&ctx, &info, pkt_len, qos_on, mirror_on, tcprt_on, tracing, now);
    }
...
#[inline(never)]
unsafe fn tc_egress_v6(ctx: &TcContext, info: &parser::PacketInfo, pkt_len: u32, qos_on: bool, mirror_on: bool, tcprt_on: bool, tracing: bool, now: u64) -> i32 {
    let ct_key = CtKey6 {
        src_ip: info.src_ip_v6,
        dst_ip: info.dst_ip_v6,
        src_port: info.src_port,
