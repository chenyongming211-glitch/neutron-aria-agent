...
| G5 production ACL source | pass for MVP | `aria_acl` extension/API/DB, REST CRUD, and `NeutronAclSource` full-resync input passed on `compute-1.example.test` and `compute-2.example.test`. |
| G6 full resync | pass for ACL MVP | `compute-1` applied five ACL-managed ports and rolled them back; `compute-2` handled the zero-compute-port case cleanly. |
...
- Compute-only hosts still need the same stage-two agent egg. `compute-3` initially
...
| High-risk lifecycle acceptance | Dual-stack fragment semantics, selector isolation, active mutation, restart, tap recreation, detach cleanup, and OVS canary evidence have explicit pass/degraded boundaries. |
