...
| REVIEW-ACL-056 | P1 | Fragment-safe ACL/CT key semantics | implementation and hosted CI complete; privileged field evidence deferred | IPv4 non-first fragments are no longer parsed as ports and IPv6 uses the same bounded context contract. The guarded field orchestration now installs direction-specific UDP/53 policy, uses an explicit unique token/ID for every independent dual-stack and dual-direction ordering scenario, proves later-before-first non-delivery, stages VLAN/tap isolation chains, asserts publication-stale and restart-scrub transitions, and records cleanup-aware fragment status. The temporary standalone fixture alone uses capacity 8 for deterministic occupancy/pressure/oldest-identity eviction evidence. | `FRAGMENT_TRACKING_SMOKE=1` is the explicit privileged execution gate. Standalone generates separate bounded VLAN-A/VLAN-B endpoint names, assigns the test addresses only to VLAN A, keeps `MODE=system` single-interface, and gives `MODE=tap` the authoritative second auto-attached tap probe. Managed execution requires an existing local peer netns/interface, exact dual-stack VLAN-A addresses, two explicit VLAN IDs, and four validated host/peer VLAN endpoint names; it preserves deployed capacity and uses real full-resync/restart operations. No privileged run was performed here: delivery and cleanup are not field-verified, evidence remains `deferred/pending`, and shipped production configuration remains disabled at capacity 8192. `update_failed` stays covered only by hosted Rust/eBPF contracts because no safe deterministic field trigger exists. |
...
| REVIEW-ACL-067 | P2 | Fragment-context failure can delete a replacement CT entry | confirmed; fix in progress | A tracked first fragment records only whether its `BPF_NOEXIST` CT insert succeeded. If that CT entry is then removed by a concurrent scrub/stale path and another CPU recreates the same key, a later fragment-context insert failure still performs an unconditional key delete and removes the replacement entry. The historical boolean no longer proves current ownership. | Add a RED interleaving regression. Reorder the tracked-first success path so fragment context is installed before CT creation, then remove the unsafe context-failure CT delete. Do not replace it with lookup-then-delete because that retains a TOCTOU race. Preserve ordinary unfragmented CT behavior. |
| REVIEW-ACL-068 | P2 | Early fragment drops reuse stale per-CPU identity fields | confirmed; fix in progress | TC packet setup reuses `PIPE_SCRATCH` but resets only part of `PipelineCtx`. Fragment resolution can drop before CT/group lookup initializes `src_id`, `dst_id`, and matched fields, so drop profiling and trace can attribute missing/expired/stale/disabled fragment drops to the previous packet handled by that CPU. | Add a RED consecutive-packet regression that seeds non-zero identity/match fields and then takes an early fragment-drop path. Fully initialize per-packet pipeline scratch before fragment resolution, including IDs, matched fields, and padding. |
...
6. **Define and implement fragment semantics:** `REVIEW-ACL-056`. The selected
   design is the enhanced Cilium fragment-context model, not steady-state
   rejection of every port-dependent fragment. Land parser metadata, bounded
   per-tap/direction/VLAN L4 context, monotonic publication epochs, recovered
   five-tuple CT, and non-initial TCPRT exclusion. Cover raw fixtures, ordered
   fragments, reordering after the first fragment, explicit later-before-first
   drops, both TC directions, IPv4/IPv6 parity, and a privileged smoke. After
   this P1 closes, finish the remaining ordinary
