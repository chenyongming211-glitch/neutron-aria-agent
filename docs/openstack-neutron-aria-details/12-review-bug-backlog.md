...
| DEBT-CI-001 | medium | verification breadth | implementation complete; first hosted audit RED | The registered Build workflow now has default-off, independent `quality-rust` and `quality-scripts` jobs. Ordinary push Build [31889696509](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/31889696509) skipped both jobs, preserving the required path. Exact implementation head `bb56310` was manually audited in Build [31890013101](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/31890013101): both jobs started concurrently; `quality-scripts` failed after 26 seconds and `quality-rust` failed after 107 seconds. Ruff correctness passed. The broad Rust test executed 487 tests and exposed two dormant test-contract defects; ShellCheck exposed 85 diagnostics at 82 locations in 24 tracked scripts. | Keep the full workspace, path inventory and strict policies unchanged. Remediate `DEBT-CI-002`, `DEBT-CI-003`, and `DEBT-CI-004` under `docs/superpowers/plans/2026-08-15-debt-ci-001-first-audit-remediation.md`, then obtain three green runs of one unchanged exact head before closing this item. Scheduled activation remains pending default-branch governance. |
...
| DEBT-CI-004 | medium | shell correctness baseline | open; first audit finding | The first full tracked-script ShellCheck run reported 85 diagnostics at 82 locations across 24 scripts: environment-assignment/expansion errors, unsafe or ambiguous quoting/control-flow constructs, deprecated commands, and variables/literals that require either correction or a narrow proof-backed annotation. Ruff correctness passed. | Repair real shell semantics first. Use line-local ShellCheck annotations only where literal expansion, intentional word splitting, or retry-loop counters are demonstrably required. Do not add global excludes, reduce the tracked file set, or weaken the quality job. Require the unchanged full ShellCheck command to pass. |
