...
| REVIEW-ACL-111 | P2 | Optional counters can escape their failure boundary | open | Status v3 currently preserves any dictionary-valued `counters` section without validating its nested schema. Malformed port/row values can fail `port_counters_blob` while `CompositeStatusReporter` deliberately runs the port reporter before the ordinary heartbeat, so enabling the optional feature can suppress that cycle's heartbeat. This contradicts the Phase B contract that counters failures are contained to `counters_error` and never affect ACL status or writes. The feature is default-off, so this is an enablement blocker rather than an active ACL-enforcement P1. | Strictly validate counters schema v1 at the UDS boundary. Convert counters-only decode/sample failures into `counters_error` while retaining the last good sample. Preserve the existing ordering and failure semantics for genuine non-counter port-status errors; do not make those errors look successful. Add malformed-section and heartbeat-containment behavior tests. |
| REVIEW-ACL-112 | P2 | Counter collection can return partial or empty success on runtime faults | open | `core/src/port_counters.rs` skips per-entry Aya iterator errors with `if let Ok`, `Path::exists()` turns metadata/permission failures into absence, and `pin_missing` does not recognize the `MapError::SyscallError` returned by Aya 0.13.1 `MapData::from_pin`. `TapRegistry::tap_runtimes_now` also maps lock contention to an empty snapshot, while `FirewallInstance::tap_id` maps every state-read error to `None`. A counter fault can therefore look like a valid zero/partial sample instead of `counters_error`. | Remove the pre-open `exists()` classification, treat only `BPF_OBJ_GET` ENOENT as absent, propagate all iterator faults, and return a typed runtime-snapshot error for lock/tap-state failures. The explicit counters response must carry the error without changing ordinary status/readiness. Add Rust behavior tests; do not add a source parser/checker. |
| REVIEW-ACL-113 | P2 | Counter reset detection covers only a subset of cumulative identity | open | `diff_port_counters` checks only three port totals and bucket/reason packets+bytes. It omits port byte/allow fields, bucket dropped fields, and `tap_id`; a map replacement or tap recreation can therefore produce negative component rates or compare unrelated counter identities while reporting `reset_detected=false`. | Treat any decrease in every cumulative summary/bucket/reason field, any tap-id change, and non-increasing sample identity as a reset. Preserve current cumulative values but emit no rates for the reset sample. Add one focused test per missing identity class. |
| REVIEW-ACL-114 | P3 | Counter latest-snapshot persistence is inconsistent across repositories | open | A clean no-sample report clears summary columns but `_persist_counter_rows` returns before deleting the previous detail rows. The stdlib `SqliteAriaAclRepository`, which is documented as preserving the public repository contract, has neither the counters table nor counter methods and serializes datetime-bearing status payloads directly through JSON. Memory, SQLAlchemy, and SQLite paths therefore disagree on latest-snapshot replacement and payload shape. | On clean absence, atomically replace details with an empty set while keeping last-good rows on explicit `counters_error`. Add counters table/method parity and JSON-safe datetime handling to the SQLite repository, with deterministic ordering shared by all repositories. |
| REVIEW-ACL-115 | P3 | Counter CLI exposes raw optional fields and erases unknown reason identity | open | `aria-acl-port-status-show` removes raw `aria_acl_port_counters` and group-map fields only when `--counters` is present, so default output can expose the large implementation payload. Unknown reason ids render only as `UNKNOWN`, losing the numeric value needed to diagnose contract drift. | Always hide raw detail/group-map fields from the default show output. With `--counters`, render the normalized rows; known reasons use stable names and unknown reasons retain `UNKNOWN(<id>)`. Add legacy CLI behavior tests. |
