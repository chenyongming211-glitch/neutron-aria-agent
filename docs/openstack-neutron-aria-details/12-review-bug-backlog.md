...
| REVIEW-OPS-039 | P2 | Map-open failures are conflated with empty state | fixed (`343446c`) | Optional pinned-map access now treats only Aya `BPF_OBJ_GET` `ENOENT` as absent. QoS rule-state and CT list paths propagate every other open, conversion, and iteration fault; a failed post-delete QoS read cannot rewrite the existing enable flag. The deliberately lenient CT flush contract from `REVIEW-ACL-053` is unchanged. | RED `ddba868` / Build `31991018142` failed on the missing exact classifier and QoS synchronization barrier. GREEN `343446c` / exact-head Build `31991207211` passed all four `map_authority_` behavior contracts and the warning-denied Rust static build. No privileged datapath result is claimed. |
