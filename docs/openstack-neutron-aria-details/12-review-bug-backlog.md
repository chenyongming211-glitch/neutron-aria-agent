...
| DEBT-CI-001 | medium | verification breadth | implementation complete; hosted audit remediation active | The registered Build workflow now has default-off, independent `quality-rust` and `quality-scripts` jobs. Ordinary push Build [31889696509](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/31889696509) skipped both jobs, preserving the required path. Exact implementation head `bb56310` was manually audited in Build [31890013101](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/31890013101): both jobs started concurrently; `quality-scripts` failed after 26 seconds and `quality-rust` failed after 107 seconds. Ruff correctness passed. The broad Rust test executed 487 tests and exposed two dormant test-contract defects; ShellCheck exposed 85 diagnostics at 82 locations in 26 tracked scripts. After those Rust tests were corrected, Build [31890412178](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/31890412178) passed the full workspace test and reached the first Clippy diagnostic, `DEBT-CI-005`. | Keep the full workspace, path inventory and strict policies unchanged. Remediate `DEBT-CI-002` through `DEBT-CI-005` under `docs/superpowers/plans/2026-08-15-debt-ci-001-first-audit-remediation.md`, then obtain three green runs of one unchanged exact head before closing this item. Scheduled activation remains pending default-branch governance. |
| DEBT-CI-002 | medium | Rust test correctness | open; first audit finding | `neutron_api::tests::counters_groups_reads_cidrs_and_sorts_by_id` creates `src-b` first and `src-a` second, then incorrectly asserts the second allocated id is smaller than the first. The production `counters_groups` implementation already sorts by numeric id; the dormant assertion itself is inverted. | Correct the fixture/assertions without changing production counter ordering, then require the broad host-workspace test to pass. |
| DEBT-CI-003 | medium | Rust test fixture fidelity | open; first audit finding | `tap_registry::tests::neutron_orphan_link_cleanup_does_not_consume_retry_marker` models an already-disappeared tap but calls `reserve_persisted_live_iface`, which correctly reads the live sysfs ifindex and therefore fails before exercising orphan link cleanup. | Seed the persisted orphan marker directly with a historical ifindex in the test fixture. Preserve production marker reservation and orphan cleanup behavior, then require the broad host-workspace test to pass. |
| DEBT-CI-004 | medium | shell correctness baseline | remediation pushed; hosted GREEN pending | The first full tracked-script ShellCheck run reported 85 diagnostics at 82 locations across 26 scripts: environment-assignment/expansion errors, unsafe or ambiguous quoting/control-flow constructs, deprecated commands, and variables/literals that require either correction or a narrow proof-backed annotation. Commit `cb582c8` repairs the real shell semantics and uses only line-local, explained annotations for literal source matching and encoded-byte argument splitting. Ruff correctness passed. | Require the unchanged full ShellCheck command to pass. Do not add global excludes, reduce the tracked file set, or weaken the quality job. |
| DEBT-CI-005 | low | Rust API structure | open; second audit finding | Once the 487-test host workspace became green, Clippy `-D warnings` reached `abi/src/fragment.rs` and rejected `fragment_resolve_decision` for eight positional arguments. This is an internal API clarity/maintenance defect, not a datapath behavior failure. | Introduce one explicit `FragmentResolveInput` value for packet/runtime decision inputs, preserve config/epoch/context references and all decisions, update every Rust/eBPF/test caller, and require workspace test, Clippy and rustfmt GREEN. Do not suppress `clippy::too_many_arguments`. |
