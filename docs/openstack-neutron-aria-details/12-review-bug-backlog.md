...
| REVIEW-ACL-057 | P1 | Direct ACL policy mutation leaves same-bank CT valid | open | Standalone/direct policy add, update, delete, and batch mutate the active ACL bank. CT validity is bank-based, so existing PASS decisions remain current after a rule becomes deny and active traffic refreshes them. This item is intentionally policy-scoped; the separately recorded referenced-group membership variant is `REVIEW-ACL-066`. | Use a full shadow-bank publish transaction for every direct/batch policy mutation, or quiesce and strictly scrub CT before an atomic mutation. Add live-flow add-deny/delete-allow regressions. Do not mark `REVIEW-ACL-066` fixed from policy-only evidence. |
...
| REVIEW-ACL-066 | P1 | Referenced standalone group expansion leaves same-bank CT valid | open; explicitly deferred after the policy-only ACL-057 batch | `add_group_standalone_locked` permits adding a CIDR to an existing group even when an ACL policy references that group, and writes the new selector directly into the active ACL source/destination bank. A previously cached PASS flow that should match a deny after the membership expansion keeps the same bank identity and can continue refreshing the stale decision. Whole-group delete is already rejected while a policy references the group, so the reproduced boundary is membership expansion rather than referenced deletion. | After `REVIEW-ACL-057` and `REVIEW-ACL-059`, route ACL-referenced standalone group membership changes through one shadow-bank publication plus strict CT invalidation, or reject the mutation while referenced. Add a live-flow regression proving expansion into a deny group invalidates the old PASS. Keep ordinary unreferenced-group persistence under `DEBT-ACL-001`. |
...
5. **Unify direct ACL publication:** `REVIEW-ACL-057`, followed by
   ...
   This batch remains policy-scoped. Then complete the separately recorded
   `REVIEW-ACL-066` referenced-group membership transaction before starting the
   fragment batch; policy-only evidence must not close the group finding.
