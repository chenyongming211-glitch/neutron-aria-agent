...
| REVIEW-TXN-040 | P2 | Blocked delete row is omitted from public Status V1 | source repair pending hosted and field verification | `build_blocked_delete_runtime` correctly retained a non-ready port row and checkpointed it with `blocked_recovery_required`, but gave that row no desired hash. `project_status_v1_ports` rejects hashless managed rows, so operators saw an empty `port_statuses` array while the exact blocked row remained present in RAM/WAL. The runtime pending identity must stay hashless; only the retained row should carry the last applied hash so it remains visible as `blocked` with ACL `unchanged` or `bypass`. | Target 4.18 isolated delete-after-purge fault injection at RC `981cfb30` produced the RED while preserving the production datapath, OVS agent, and `br-int` inventory. A focused Rust projection regression and a repeat of all four isolated delete fault fixtures are required before closure. |
