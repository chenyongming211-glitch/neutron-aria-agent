...
| RISK-ENV-001 | high | Clustered Neutron API/DB view during node recovery | open; root cause pending node recovery | During the 2026-08-07 low-impact continuation, the clustered API intermittently returned one already-cleaned synthetic policy in collection output while repeated item and collection reads later returned not-found/zero. Direct reads through the active local neutron-server were consistently zero. Another node was under operator recovery and was deliberately not accessed, so backend or database synchronization could not be proven. | Keep the recovering backend out of production API/DB rotation until its service and database state are verified. After operator recovery, identify every API backend, compare the same policy/list query directly on each backend, verify database cluster synchronization, then repeat create/read/delete with a correlation ID through both direct and load-balanced endpoints. Do not classify this as an Aria repository bug or close it as transient without that evidence. |
...
This register retains all 85 stable `REVIEW-*` IDs. Use the `Status` column,
...
| REVIEW-ACL-071 | P2 | Composite port-status ID is not path-safe on target Neutron 9 | open | The API returns exact per-host IDs in the form `aria-status-v1.<base64url>`, but the target legacy Neutron route treats the dot suffix as a response-format extension. A field `GET` using the returned ID reached the plugin as only `aria-status-v1` and returned 404. Legacy `GET/DELETE` by a unique `port_id` still works, but exact item access cannot disambiguate multiple host rows as designed. | Reproduced on 2026-08-07 with a synthetic status row and no real port or datapath impact. The stale/read-refresh behavior passed after using the documented legacy `port_id` path, and all synthetic rows were removed. Replace the public ID with a route-safe versioned encoding or add an old-router-compatible translation, preserve marker ordering and compatibility, add controller-level route tests rather than plugin-only tests, and rerun exact show/delete on the target API. |
