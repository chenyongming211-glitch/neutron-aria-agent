...
| DEBT-CI-001 | medium | verification breadth | open; design approved | CI runs Python/stage gates, selected Rust Neutron tests, eBPF and static builds, but has no full host-workspace test job and no dedicated clippy, rustfmt, Python correctness-lint, or shellcheck audit. The approved first stage adds two parallel manual-only quality jobs to the existing registered Build workflow; ordinary push/release timing is unchanged, and no nightly claim is made until the definition reaches the default branch. | Formal design: `docs/superpowers/specs/2026-08-15-debt-ci-001-quality-audit-design.md`. After `RISK-CI-001`, write a separate RED/GREEN implementation plan. Run host workspace tests/clippy excluding only the BPF cdylib, format all Rust, and run pinned Ruff correctness rules plus Shellcheck over every tracked shell script. Promote checks only after three exact-head green runs and measured duration evidence. |
...
| RISK-CI-001 | medium | workflow supply chain | open; immutable-pinning design approved | Checkout, cache, download and release actions are pinned, but two `dtolnay/rust-toolchain@stable`, one `@master`, and five `actions/upload-artifact@v4` executions remain mutable. The approved repair pins all eight, makes stable/nightly toolchain selection explicit, and adds a Cargo-free mutation-tested validator requiring every external workflow `uses:` reference to carry a full lowercase 40-character commit SHA. | Formal design: `docs/superpowers/specs/2026-08-15-risk-ci-001-immutable-action-pins-design.md`. RED must fail against the current eight refs; GREEN must preserve toolchain, permission, artifact and release behavior in exact-head hosted CI. Dependabot cannot be honestly activated from this non-default delivery branch because GitHub reads its configuration from the default branch; reviewed manual refresh remains the interim policy and default-branch automation remains a governance follow-up. |
