...
| REVIEW-ACL-086 | P2 | Cross-CPU CT entry alias through preallocated LRU reuse | open, target-source confirmed | `ebpf/src/conntrack.rs:143-249` retains mutable `CT_TABLE_V4/V6` value pointers while other CPUs can delete or evict the same entry. Exact `4.18.0-553.5.1.el8_10` source proves `htab_lru_map_delete_elem()` immediately calls `bpf_lru_push_free()`, and `prealloc_lru_pop()` reuses that node and overwrites its key/value without an RCU grace period; the same kernel rejects LRU plus `BPF_F_NO_PREALLOC`. A retained pointer can therefore alias another flow and affect lifecycle state or the cached policy read, not only counters. | Use two complete per-CPU snapshots of the same requested key, reject inconsistent/missing confirmation as an ordinary miss, mutate only scratch, and publish the full value with key-scoped `BPF_EXIST`; never write through the retained CT pointer. Formal design: `docs/superpowers/specs/2026-08-15-review-acl-086-ct-lru-concurrency-design.md`. RED/GREEN and hosted eBPF/stack evidence pending; exact-kernel stress remains deferred field evidence. |
