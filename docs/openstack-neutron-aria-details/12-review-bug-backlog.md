...
| IDs | Evidence state |
| --- | --- |
| `ACL-055` | **Likely fixed, not operationally closed.** Neutron-managed ACL/CT now uses bank-aware TC ingress and egress; TC-mode XDP bypasses ACL/CT; routine CT diagnostic events require a matching Trace filter while stale-bank remains unconditional; enforcement publication requires both TC links. GREEN Build [29202917780](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29202917780) passed the exact Rust metric test, nightly eBPF build, static userspace/agent builds, binary verification, and Python stage gates. `real-tap smoke pending`: a successful managed-tap `summary.json` is still required before changing this item to `fixed`. |
...
- Fix commit `bfa2d1e77be1b6ba93e7cdee3aec6f46adf4be0f` passed complete Build
  ...
  static userspace/agent builds, binary verification, and all Python stages.
