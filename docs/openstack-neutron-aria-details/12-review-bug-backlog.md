...
authoritative Register below now records `REVIEW-OPS-019` fixed and
`REVIEW-ACL-045` source/hosted-CI complete. ACL-045 remains open only because
its residual-map proof still requires privileged runtime evidence.
...
| --- | --- | --- | --- | --- | --- |
| RISK-READY-001 | high | readiness boundary | open; source implementation and hosted CI complete; deployment/field wiring deferred | Commit `9060a77` adds UDS-only `GET /readyz` without changing TCP `/api/v1/health` liveness. `/readyz` and `/api/v1/neutron/status` now build the same full Status V1 response from one `NeutronApiState`; only exact `overall_readiness=ready` returns HTTP 200, while cold-start/pending `unknown`, terminal `degraded`, and recovery/operator `blocked` return HTTP 503. The route is recorded in the versioned UDS contract and thin Python client, and is not exposed through TCP OpenAPI. | RED `7447e4e` / Build [30707303054](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30707303054) failed on the missing readiness handler. GREEN `9060a77` / exact-head Build [30707571086](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30707571086) passed both strict readiness tests, selected Rust behavior, and warning-denied eBPF/userspace/static builds. Keep the risk open: target-environment deployment wiring, Neutron RPC heartbeat composition, recovery timing, and rollback behavior remain `deferred/pending`; no field PASS is claimed. |
...
This register retains all 84 stable `REVIEW-*` IDs. Use the `Status` column,
...
| REVIEW-ACL-013 | P3 | Neutron port extension projection | implementation and hosted CI complete; target field evidence deferred | Commit `133b52b` installs a batch-aware legacy `get_port`/`get_ports` projection. It computes desired fields from one effective snapshot, queries runtime rows once for the returned port IDs, accepts only the current `(port_id, binding:host_id)` plus current policy/binding identity, maps fresh `ready` to legacy `applied`, and makes stale/missing/mismatched evidence conservative. Projection failure returns a complete unknown summary without failing the native port read. RED `211f7fa` / Build [30703496690](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30703496690) failed the four missing projection behaviors; GREEN `133b52b` / Build [30703680367](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30703680367) passed all hosted lanes. | Commit `0b32d4b` wires REST and real legacy `neutron port-show` checks into the existing CLI consistency smoke; Build [30703735814](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30703735814) passed the complete required build. Commit `f4d3a51` adds Python 2.7 clean-container method-binding coverage; Build [30703793706](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30703793706) passed that lane plus warning-denied Rust/eBPF/static builds. Documentation head `e637301` passed Build [30703900873](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30703900873). No target environment was used: keep the row open and evidence `deferred/pending` until the smoke runs on the product Neutron 9/Python 2 image. |
...
| REVIEW-ACL-045 | P2 | Orphan reconcile skips map scrub | implementation and hosted CI complete; privileged field evidence deferred | Commit `8242c1b` derives orphan identity from the union of XDP/TC link pins and persisted live-iface markers, subtracts committed siblings, loads the stable tap id from strict per-interface state, and serializes cleanup with the iface and runtime lifecycle locks. Link removal no longer consumes the retry marker. The concrete control-plane path removes kernel-drop bindings by tap id, unregisters trace state, calls the real full `scrub_managed_runtime_state` across both ACL banks and all tap-scoped runtime families, clears stale authority/registry state, then releases the marker last. Any required failure reports blocked and leaves the marker/state identity for idempotent startup retry. | RED `a9a536e` / Build [30613528175](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30613528175) proved the missing inventory, stable-id, and marker-last boundaries. Exact-head GREEN `8242c1b` / Build [30613890526](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30613890526) passed `fast-contracts`, Rust behavior tests, warning-denied Rust/eBPF build, and static verification. Keep the finding open until privileged evidence verifies every pinned map family, real links, kernel-drop binding, trace state, marker removal, committed-sibling preservation, and failure retry. Distinct from `REVIEW-ACL-035`. |
| REVIEW-ACL-046 | P1 | Cross-domain ACL selector isolation | privileged standalone tap evidence complete; managed rollout remains gated | Reopened 2026-07-15 with a complete enforcement path. The repair derives ACL maps from final direction-specific rule references, uses a conflict-aware general projection, gates ownership/skip on projection health, and repairs legacy pollution through bank publication plus strict CT invalidation while preserving standalone direct publication for `REVIEW-ACL-057`. The transaction implementation is `49081c6`. Pre-field wiring/hardening commits `d1aa523..ad30cad` cover managed detach ordering, purge-failure atomicity, strict-flush rollback, and successful retry detach; independent final review approved the wiring. Exact-head GitHub Actions run [29672271181](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29672271181) at `ad30cad` passed `fast-contracts`, `rust-behavior`, and `rust-build`. | A privileged legacy-kernel standalone tap fixture passed selector-versus-general-group isolation on 2026-08-06. The sanitized evidence is in `docs/evidence/openstack-n05-lite/20260806-acl-high-risk-field-acceptance/summary.md`. This closes the missing enforcement-path evidence for the fixture, but production managed rollout remains controlled by the wider ACL lifecycle and activation gates. |
...
| REVIEW-ACL-060 | P2 | ACL list pagination and N+1 queries | fixed | All five list resources share strict typed filters, deterministic identity-tied keyset ordering, forward/reverse markers, field projection, and bounded native SQL/SQLite execution. Address-set members are batch-loaded once per selected page; the agent requests bounded ACL pages independently from port reads. | Commits `a46c11d`, `0087e7d`, `0dbd476`, `f9da01e`, `5a7845b`, and `3999e49`. Exact-head Build [30644674860](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30644674860) passed 543 fast contracts, 9 CLI contracts, and 4 warning-as-error DB contracts. SQL budgets are 1 normal page, 2 custom-marker page, 1 address-set page without members, and 2 with members. Target Neutron 9/Python 2/SQLAlchemy 1.0 field evidence remains deferred/pending. |
...
| REVIEW-ACL-064 | P1 | Detach purge active-policy-miss PASS window | successful detach path field-verified; injected failure path pending | Commit `49081c6` replaces item-at-a-time detach purge with the ordered transaction: quiesce ACL/CT, publish an empty owned ACL projection, strictly flush CT, then detach; purge or flush failure keeps the interface attached and quiesced. Later lifecycle repairs make already-deleted tap observation and delete recovery idempotent. | A clean Neutron port detach on 2026-08-06 removed the tap, converged runtime/generation state, and cleared ACL ownership while the independent OVS canary recorded zero loss. See the sanitized high-risk field summary. Keep the finding open until privileged fault injection proves purge/flush failure while the tap still exists cannot create a policy-miss PASS window. |
| REVIEW-ACL-065 | P1 | Privileged purge partial owned-ACL deletion | successful detach path field-verified; injected failure path pending | Commit `49081c6` removes privileged item deletion in favor of one quiesced empty-owned-ACL publication with complete rollback; failure aborts detach and preserves the complete prior owned state. This remains the root cause beneath the older ignored-error symptom in `REVIEW-ACL-023`. Later lifecycle repairs make already-deleted tap cleanup and full-resync recovery idempotent. | A clean Neutron port detach on 2026-08-06 converged with no residual target-port ownership and zero independent OVS canary loss. See the sanitized high-risk field summary. Keep the finding open until privileged fault injection proves a purge failure preserves the complete old owned state and a later retry detaches cleanly. |
...
| REVIEW-OPS-036 | P3 | XDP pinned-path health can false-pass | implementation and hosted CI complete; privileged field evidence deferred | `FirewallInstance`, standalone startup, and shared-runtime recovery now use one in-process observation of the exact pinned program and link. Readiness and ownership require XDP link type, nonzero link ID and ifindex, the current expected interface ifindex, and the exact pinned program ID. Missing, detached, mismatched, or unverifiable evidence is not-ready; an existing unverified pin is preserved and blocks replacement while independent TC ACL/CT startup continues. | RED `c82e18e` / Build [30872857520](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30872857520) failed on the deliberately missing identity boundary. GREEN `31dcf49` / exact-head Build [30873163705](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30873163705) passed all five exact-identity behaviors plus warning-denied eBPF/userspace/agent builds. Field wiring `6548272` / Build [30873611591](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30873611591) adds an opt-in detached-but-pinned scenario with stable SKIP semantics; it has not run on a target kernel and remains `deferred/pending`. Full DDoS readiness still requires attach-mode, domain-generation, and required-map validation. |
...
8. **Source complete; privileged ACL-045 evidence deferred — close transaction
   ...
   The independent `REVIEW-ACL-045` source repair is delivered in `8242c1b`;
   exact-head Build
   [`30613890526`](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/30613890526)
   is GREEN. Keep ACL-045 open as privileged field evidence deferred. Do not
   fold the remaining production defects into one implementation commit.
