...
6. **Next implementation batch — remaining standalone group durability:**
   finish ordinary unreferenced-group strict persistence and rollback under
   `DEBT-ACL-001`. ACL-056 privileged smoke remains a separate production-
   activation gate and stays `deferred/pending`; it does not block beginning
   this next source batch or justify claiming field verification.
