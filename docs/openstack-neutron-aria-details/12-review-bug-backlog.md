...
| REVIEW-OPS-041 | P1 | Explicit Python agent config read failure silently selects defaults | open; reproduced during target RC regression | `openstack/neutron_aria/neutron_aria/agent/config.py::load_config()` ignores the filename list returned by Python 2.7 `ConfigParser.read()`. A missing or unreadable explicitly requested file therefore produces the default `heartbeat_only` configuration instead of a startup error. The normal daemon degrades instead of applying the intended ACL configuration; the maintenance `--once` path is more severe because it invokes `full_resync()` directly and can submit an empty default snapshot. The target probe reproduced one empty snapshot; restarting only `neutron_aria_agent` restored the authoritative snapshot, without restarting OVS or the datapath. | Fail closed when an explicitly supplied config path is missing or unreadable, before runtime initialization or snapshot submission. Add Python 2.7 coverage for missing, permission-denied, malformed, valid, daemon, and `--once` paths. Preserve defaults only behind an explicit no-config development mode. This is independent of Rust `REVIEW-OPS-038`. |
