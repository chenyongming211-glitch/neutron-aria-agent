...
| REVIEW-ACL-124 | P1 | Tap recreate can retain false `ready/enforce` while the replacement ifindex is temporarily unprotected | open; field-reproduced at RC `6144bf36` | A controlled VM soft reboot removed the old tap and recreated the same interface name with a new ifindex. Two of three runs admitted traffic that the active ACL had blocked before reboot while the northbound port status still reported `ready/enforce`. In the measured run, the new ifindex appeared, the guest began admitting the blocked probe 13.267 seconds later, and admission continued for the remaining 14.759-second observation window. A third run reconciled before the guest became reachable and admitted no blocked-source probes in 120 samples, proving this is an ordering race rather than a permanent failure. Independent OVS canaries were lossless in every run. | Treat `RTM_DELLINK`, missing tap, and current-ifindex mismatch as immediate attachment-identity invalidation; publish `degraded/bypass` until the replacement tap has verified ingress and egress TC identities plus the desired ACL generation/hash. On `RTM_NEWLINK`, schedule an immediate same-port reapply even when generation/hash are unchanged; do not wait for periodic full-resync. Add deterministic old-ifindex/new-ifindex behavior tests and a field smoke that records the complete enforcement-gap and status timeline. The availability-first contract may preserve OVS forwarding, but it must not report false readiness. |
