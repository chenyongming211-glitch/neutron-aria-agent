...
| REVIEW-ACL-056 | P1 | Fragment-safe ACL/CT key semantics | design approved; written specification pending review | IPv4 non-first fragments are parsed as if payload bytes were TCP/UDP ports, while IPv6 non-first fragments use zero ports. Port ACL and CT keys can diverge across fragments. The previously considered steady-state rule that rejected every port-dependent fragment does not meet the product requirement. | Implement the approved enhanced Cilium model in [`2026-07-19-acl-056-fragment-tracking-design.md`](../superpowers/specs/2026-07-19-acl-056-fragment-tracking-design.md): a valid allowed first fragment establishes bounded per-tap/direction/VLAN L4 context; later fragments recover the same ports and CT tuple; first-before-context misses, stale epochs, overlaps, expiry, and resource failures drop explicitly. A monotonic fragment epoch prevents stale reuse across repeated bank rotations. Full reassembly and buffering a later fragment before the first remain out of scope. |
