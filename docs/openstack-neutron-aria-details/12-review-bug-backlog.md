...
A 2026-08-20 target-kernel delete-fault run then recorded
`REVIEW-TXN-040`: the retained blocked row was durable in the WAL but omitted
from public Status V1 because its row-level desired hash was empty.
...
| RISK-ACL-001 | medium | dual-stack priority namespace | implementation complete; local verification passed; hosted/field migration pending | The product contract now scopes enabled rule uniqueness and effective indexing by `(policy, direction, ethertype, priority)`. IPv4 and IPv6 rules may reuse one priority, while same-family duplicates remain rejected. The new Alembic revision backfills legacy null family values as `IPv4`, replaces the DB unique index, and refuses an unsafe downgrade when cross-family reuse exists. | Run the hosted Python gates and the legacy Neutron DB migration smoke before marking fixed. Verify create/update/delete and effective status with equal IPv4/IPv6 priorities on a test policy. |
| RISK-ACL-002 | low | CIDR input symmetry | implementation complete; local verification passed; field API/CLI evidence pending | Rule CIDRs and address-set members now accept bare IPv4 and IPv6 host inputs and persist canonical `/32` and `/128` forms. Existing rejection remains for leading-zero IPv4, zones, mapped IPv6, family mismatch, embedded whitespace, and invalid prefixes. | Run API and legacy CLI field smoke for create/show/update and address-set deduplication before marking fixed. |
...
- `REVIEW-ACL-116`: legacy family-zero migration has no managed/standalone
  ...
  migration contract was subsequently repaired by the authority-aware state
  and WAL migration series recorded in the authoritative table below.
...
- `REVIEW-TXN-039`: startup prepares/deletes dormant ACL pins and publishes
  ...
  migration, contrary to the approved migration commit order. This ordering
  defect was subsequently fixed by making durable managed-state migration a
  prerequisite for runtime-schema mutation.
...
- `RISK-ACL-001`: the approved contract is family-qualified priority identity;
  enabled IPv4 and IPv6 rules may reuse `(direction, priority)`, while the DB,
  repositories, and effective index reject duplicates inside one family.
- `RISK-ACL-002`: the approved symmetric input contract accepts bare hosts and
  persists canonical IPv4 `/32` and IPv6 `/128` prefixes for rules and address
  sets.
...
| REVIEW-OPS-039 | P2 | Map-open failures are conflated with empty state | fixed (`343446c`); field verified | Optional pinned-map access now treats only Aya `BPF_OBJ_GET` `ENOENT` as absent. QoS rule-state and CT list paths propagate every other open, conversion, and iteration fault; a failed post-delete QoS read cannot rewrite the existing enable flag. The deliberately lenient CT flush contract from `REVIEW-ACL-053` is unchanged. | RED `ddba868` / Build `31991018142` failed on the missing exact classifier and QoS synchronization barrier. GREEN `343446c` / exact-head Build `31991207211` passed all four `map_authority_` behavior contracts and the warning-denied Rust static build. Target 4.18 isolated evidence at exact RC `f8ef09a` proved that a wrong pinned object returns an operational error while a missing optional pin remains an empty result; see `docs/evidence/openstack-n05-lite/20260817-legacy-family-migration-three-node-rc/summary.md`. |
...
| REVIEW-OPS-042 | P1 | Field Kolla image omits required offline Python dependency | fixed (`f8ef09a`); field verified | The legacy field base provides `netaddr 0.7.18`, while the agent requires at least 0.7.19 and the target has no Python 2 pip or Internet dependency path. The release bundle now carries a pinned universal wheel, verifies its SHA-256, installs it offline into Python 2 site-packages with Python 3 pip, and executes the real agent entry point during build and RC install. Nested artifact names remain traversal-safe and bundle-root checksums use their actual paths. | RED was the exact-source field image startup failure (`DistributionNotFound`). GREEN exact-head Build `31995871565` passed 729 fast contracts, the Python 2.7 clean-container gate, release governance and reproducibility checks, and produced the corrected bundle. All three field computes loaded the wheel hash from that bundle and reached ready/non-degraded heartbeat; see the 2026-08-17 three-compute evidence. |
| REVIEW-TEST-001 | P2, test harness | CirrOS egress UDP probe false-fails before ACL apply | fixed; field re-run passed | The active-matrix egress probe piped a nonce into BusyBox `nc -u` and closed stdin immediately. On the field guest, `nc` exited before reading the echoed datagram, so the baseline failed even though the host received traffic and no ACL object existed. The probe now writes a newline and keeps stdin open for one second before EOF; an allow returns the exact nonce, while a drop still returns no nonce. | A focused static contract failed before the change and passes after it. The original field failure occurred at `baseline_matching_allow` with no policy/rule/binding; the corrected three-node RC run passed UDP egress create, update, rule/binding/policy enable-disable, rollback, and cleanup. The original failed directory is retained only as harness-root-cause evidence and is not counted as a product failure. |
...
| REVIEW-ACL-116 | P1 | Legacy family-zero migration lacks authority context | fixed (`a29d21a..d42b339`); field verified | `LegacyAclMigrationAuthority` is now mandatory at every durable state and WAL migration boundary. Managed legacy wildcard records become IPv4-only, standalone wildcard records expand to both families, and concrete CIDRs infer their family. Replay projection mode remains independent from migration authority, and read-only tap discovery no longer performs an authority-free migration. | RED `4196ad3` required managed/standalone wildcard, concrete-CIDR, WAL replay, and checkpoint-idempotency behavior. GREEN exact-head `d42b339` / Build `31990398188` passed the focused Rust behavior lane and warning-denied Rust static build. Target 4.18 isolated evidence at RC `f8ef09a` proved managed wildcard to IPv4-only and standalone wildcard to both families, followed by a controlled three-compute Status V2-to-V3 rollout and real ACL regression; see the 2026-08-17 evidence summary. |
...
| REVIEW-TXN-039 | P2 | Runtime schema publication precedes durable family migration | fixed (`149d3e2`); field verified | Startup first refuses future schemas, then discovers and migrates every managed per-interface core-state/local-WAL pair. Only after all durable migrations succeed may dormant pins be removed or current runtime-schema metadata be published. The standalone `system` state remains outside this managed preparation boundary and retains its own explicit authority. A crash after durable migration but before runtime publication retries idempotently. | RED `4c2606d` / Build `31990548889` failed on the absent phase boundary. GREEN `149d3e2` / exact-head Build `31990801445` passed migration-failure, post-migration crash-retry, future-schema non-mutation, and live-old-schema refusal contracts plus the warning-denied Rust static build. Target 4.18 evidence proved failed durable migration leaves the old runtime schema intact, live old-schema admission is refused, and controlled quiesce plus migration restores all three computes at Status V3 with equal accepted/applied generations and zero WAL replay failures. |
| REVIEW-TXN-040 | P2 | Blocked delete row is omitted from public Status V1 | fixed (`2ab480cb`, gate `6144bf36`); field verified | `build_blocked_delete_runtime` correctly retained a non-ready port row and checkpointed it with `blocked_recovery_required`, but gave that row no desired hash. `project_status_v1_ports` rejected the hashless managed row, so operators saw an empty `port_statuses` array while the exact blocked row remained present in RAM/WAL. The runtime pending identity remains hashless; the retained row now carries the last applied hash and is visible as `blocked` with ACL `unchanged` or `bypass`. | RED target-4.18 fault injection at RC `981cfb30` omitted the row. Exact-head Build [32334693695](https://github.com/chenyongming211-glitch/neutron-aria-agent/actions/runs/32334693695) passed Rust behavior, warning-denied build, Python 2.7 and eBPF stack gates for `6144bf36`. The isolated target-4.18 `transaction-8` run passed detach ordering, purge atomicity, strict CT flush rollback and startup forward recovery; production datapath/OVS identities and `br-int` inventory were unchanged. |
| REVIEW-TXN-041 | P3, test harness | Delete fault smoke retries through the unresolved-pending write guard | fixed (`e6976edb`); field verified | The managed transaction smoke still retried DELETE in the same process after publishing a durable blocked delete checkpoint. `REVIEW-TXN-036` correctly rejects every mutation while `pending_generation` is unresolved, so the smoke failed with `delete_blocked_by_unresolved_pending` even though the product matched its contract. The smoke now restarts the isolated datapath without fault injection, verifies WAL startup forward recovery removes the managed port and pinned state, then verifies a repeated DELETE is the idempotent `not_found` response. | RED `transaction-7` reached the real 409 guard after its blocked RAM/WAL assertions passed. GREEN `transaction-8` passed all four transaction fixtures and cleanup checks with no production container, OVS agent or `br-int` identity change. Static smoke-contract checks pin the startup-recovery and idempotent-delete semantics. |
...
| REVIEW-ACL-124 | P1 | Tap recreate can retain false `ready/enforce` while the replacement ifindex is temporarily unprotected | fixed (`d4dadb32`); field verified | A controlled VM soft reboot removed the old tap and recreated the same interface name with a new ifindex. The root cause was twofold: link loss did not immediately invalidate the committed attachment identity, and the process-local replay cache rejected a valid Neutron snapshot because its logical empty `ifname` did not equal the OVS-resolved tap name. Link lifecycle events now preserve Neutron authority, publish `degraded/bypass` on identity loss, and replay the exact last successful port snapshot through the existing port-scoped WAL/apply path. The cache takes ACL content from the authoritative snapshot and tap identity from the successfully committed runtime port. | RED `60378f58` / run `32374346026` failed the logical-port cache fixture. GREEN `d4dadb32` / run `32374731339` passed the full Rust workspace (`531/531`), behavior, eBPF stack, packaging, and Python gates. The EL 4.18 field gate changed ifindex in three soft-reboot cycles, observed port-scoped replay in 3.581-5.523 seconds across calibration and authoritative runs, reported zero stable false-ready samples, admitted zero ACL probes, retained ingress/egress TC identities, and passed the authoritative OVS canary `1712/1712` without changing OVS or OVS-agent identity. Evidence: `docs/evidence/openstack-n05-lite/20260821-review-acl-124-tap-recreate/summary.md`. |
| REVIEW-ACL-125 | P1 | Shelve/offload can strand the Python snapshot transaction after Rust detaches an absent tap | fixed (`ba9e7c90`); field verified | Legacy Neutron retains the old host binding while a shelved VM tap is absent. Rust correctly detached the old runtime and emitted an exact detached tombstone, but Python still required every logical candidate in `managed_ports`. It rejected the committed generation, retried with newer generations, and later refused to replace the unresolved local pending snapshot after unshelve changed desired state. | Python now accepts a detached candidate only for `pending_local_validation` with no supplied interface identity and an exact current-generation/current-hash tombstone that passes the existing detached-domain checks. Historical, mismatched, and unrelated tombstones remain diagnostic only. Target Python 2.7 tests passed, the three-compute Kolla rollout passed, and the repeated shelve/offload plus unshelve cycle remained ready with no pending generation. Two independent OVS canaries totalled `4551/4551` replies with zero loss. Evidence: `docs/evidence/openstack-n05-lite/20260821-kolla-rc-lifecycle/summary.md`. |
| REVIEW-OPS-043 | P1 | Legacy Neutron HTTP requests have no independent timeout | fixed (`00f03a59`); field and hosted verified | On the three-compute RC control-plane gate, a TCP peer that accepted the Keystone/Neutron connection but never returned an HTTP response blocked the legacy `python-neutronclient` call beyond the external 10-second probe. The installed client default was `timeout=None`, so a single stalled API request could stop full resync and heartbeat progress indefinitely. | Commit `00f03a59` adds the positive `[neutron] api_timeout` contract with a conservative `10.0s` default and passes it to the port source, ACL source, and port-status client. The target Python 2.7 RC exited a real HTTP blackhole in `10.730s`, recovered a real API read in `0.944s`, and remained healthy on all three computes without restarting datapath or OVS components. Exact evidence-head Build [32455870722](https://github.com/chenyongming211-glitch/neutron-aria-agent/actions/runs/32455870722) passed the full hosted gate. Field evidence: `docs/evidence/openstack-n05-lite/20260821-control-plane-fault-acceptance/summary.md`. |
| REVIEW-OPS-044 | P3 | Live ACL smoke transports an unbounded port-status list through one environment variable | open | The current RC applied and enforced the ACL correctly, but `assert_port_status_identity` exported the complete list response as `STATUS_PAYLOAD`. At the field database size this exceeded Linux `ARG_MAX`, so the smoke exited `126` after datapath validation. Its cleanup trap succeeded and a rerun with the optional identity check disabled passed the full traffic and rollback path. | Stream the JSON payload to the parser through stdin or a root-owned temporary file, retain exact policy/binding identity assertions, and add a large-payload regression. This is a test-tool defect, not an ACL enforcement failure. |
