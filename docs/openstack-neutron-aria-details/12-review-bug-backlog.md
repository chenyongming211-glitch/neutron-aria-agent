...
| REVIEW-ACL-075 | P1 | TC parse-uncertainty fail-open bypass | open | Commit `8bab0b8` deliberately returns `TC_ACT_OK` when `parse_tc_packet` fails (`ebpf/src/lib.rs:118-123`, `314-319`), replacing the former drop path. Two kernel-valid packet shapes therefore skip ACL/CT/QoS evaluation entirely: an IPv4 first fragment whose TCP/UDP header is truncated (`parser.rs:243-246`) and an IPv6 packet carrying more than four extension headers (`parser.rs:345`, `390-392`); RFC 8200 treats long extension-header chains as valid, so an attacker can run an entire connection uninspected. | Re-review the fail-open decision: distinguish adversarial parse failure from expected runtime uncertainty. Either fail closed for structurally valid packets the parser cannot walk, or raise the extension-header walk limit within verifier-safe bounds and route truncated first fragments through the fragment-decision path instead of a pass. Add parser-contract tests for both shapes. |
...
| REVIEW-ACL-080 | P2 | Identical partially-failed snapshot re-submit never retries | open | `pending_snapshot_submit_response` (`agent/src/neutron_api.rs:2303-2335`) returns `pending` for the same generation+hash without re-attempting failed ports, and `build_snapshot_commit_runtime` (`:3868-3870`) keeps them pending. A transient per-port failure therefore never converges without operator-driven recover-pending plus a new generation. | Retry failed per-port operations idempotently on an identical re-submit, or document and enforce a driver-visible retry contract; add a transient-failure re-submit test. |
| REVIEW-ACL-081 | P2 | Dead XDP link is never projected into per-port Neutron status | open | `project_tc_acl_link_loss` (`agent/src/neutron_api.rs:1086-1164`) skips attach-only ports (`:1101-1106`), skips ACL ports with healthy TC links (`:1110-1112`), and only accepts `missing_tc_*` / `acl_quiesce_failed:` reasons (`:1116-1120`), so the `xdp_ddos_hook_unavailable` readiness reason never reaches Neutron per-port status even though the HTTP instances endpoint reports `xdp_ready=false`. | Project XDP-link loss into per-port status or document the boundary explicitly; add a reason-filter regression test. |
...
| REVIEW-ACL-083 | P2 | Shared fallback in-memory ACL repository mixes requests and loses state | open | `plugin.py:53-54` creates one `InMemoryAriaAclRepository` per process and `_repo` (`:479-489`) serves it whenever `context.session` is absent, mixing tenant data across requests and silently losing all ACL configuration on restart. List and delete methods are additionally unlocked while mutations take `_locked_write` (`db/aria_acl/api.py:292-513`). | Fail closed (or raise a configuration error) when the DB session is unavailable instead of silently serving an in-memory store, and lock the remaining repository methods. |
| REVIEW-ACL-084 | P3 | DB write errors inside a pre-existing transaction skip rollback | open | `_write_transaction` (`db/aria_acl/api.py:1041-1056`) yields without rollback when the session already has a transaction, and `_neutron_write` (`:189-203`) re-raises; the intended 409/400 mapping can leave the outer request transaction in a broken state. The end-to-end HTTP consequence was not traced and is recorded as the mechanism only. | Roll back or mark the session dirty on inner errors in the pre-existing-transaction branch; add a fault-injection test. |
...
| REVIEW-ACL-086 | P2 | Cross-CPU CT entry use-after-free through get_ptr_mut | open | `ebpf/src/conntrack.rs:143-158` (and the v6/reverse equivalents) take a raw element pointer then `remove(key)` on stale/expired paths; a concurrent CPU holding the same slot pointer can write `last_seen`/counters through a slot the remove freed for reuse, corrupting an unrelated flow entry. | Re-lookup or re-check the entry under a single guarded sequence, or use per-CPU-safe updates; document the accepted race if tolerated. |
| REVIEW-ACL-087 | P2 | Truncated first fragment passes while successors are dropped | open | A first fragment whose L4 header is truncated fails parse and passes (`ebpf/src/lib.rs:118-123`, `parser.rs:243-246`) without installing a fragment context, so every non-initial fragment of the same datagram is dropped for missing context (`ebpf/src/fragment.rs:139-144`). Reassembly can never complete, but the first fragment is admitted un-inspected and the drop asymmetry is a DoS surface. | Route truncated first fragments through the fragment-decision path (record context or fail closed) instead of the parse-failure pass. |
| REVIEW-ACL-088 | P2 | delete_network ignores its id argument | open | `delete_network` (`core/src/ebpf_ops/network.rs:182-190`) builds the LPM key purely from `(tap_id, ip, prefix)` and never checks the stored owner id; with two groups sharing a CIDR the second insert overwrites the first (`BPF_ANY`), and removing the CIDR from one group destroys the other's entry. Current agent callers guard via `capture_network_owner`, but the core API is a silent-corruption footgun. | Read back the stored id and refuse mismatched deletes, or remove the unused parameter; add a two-group shared-CIDR test. |
...
| REVIEW-ACL-091 | P3 | TypeError-based signature sniffing can double-issue writes | open | `agent/neutron_client.py:133-136` and `agent/status_reporter.py:259-266`, `312-315` retry POST/DELETE with an alternate signature on any `TypeError`, which cannot distinguish a signature mismatch from a `TypeError` raised during response processing; a side-effecting request can be issued twice. | Probe callable signatures once at construction instead of retrying on `TypeError`; add a double-issue test. |
| REVIEW-ACL-092 | P3 | commit_delete leaves deleted ports in feature-ready history | open | `commit_delete` (`agent/state.py:573-587`) prunes only `last_classified_projected_port_ids`; `last_feature_ready_projected_port_ids` is never pruned, so deleted ports keep appearing in `feature_ready_history` output until the next full snapshot commit. | Prune the feature-ready track on `commit_delete`; add a regression test. |
...
| REVIEW-ACL-095 | P3 | Managed-tap config updates silently drop ssl_enabled | open | `update_runtime_config` (`core/src/ebpf_ops/runtime.rs:237-258`) passes no ssl field for managed taps (`TapConfig` has none), so an update carrying `ssl_enabled` succeeds while discarding the value without warning. | Reject or document the unsupported field for managed taps; add a contract test. |
...
| REVIEW-TXN-033 | P1 | WAL compact snapshot-before-truncate window corrupts the bitmap allocator | open | `WalWriter::compact` (`core/src/wal.rs:200-236`) renames the new snapshot before truncating the WAL. A crash between the two replays the full WAL prefix on top of the new snapshot; `apply_add_rule` (`core/src/state.rs:462-512`) is not idempotent for updates, so replayed older `AddRule` entries release the final rule's port set and allocate fresh indices, diverging in-memory `bitmap_idx` from the pinned kernel port pool. | Truncate and fsync the WAL before renaming the snapshot, or make rule replay idempotent against snapshot contents; add a compact-crash replay test asserting allocator/bitmap stability. |
...
| REVIEW-TXN-035 | P2 | Startup reconcile masks per-port error status after partial apply | open | `reconcile_committed_runtime` (`agent/src/neutron_api.rs:624-756`) rebuilds every committed port as `ready` at `applied_generation` without consulting its prior status and without a `pending_generation` gate (the gate exists only on the periodic path). After a partial apply, ports whose update failed with status `error` (`:3259-3271`) are rewritten to `ready` at the old generation on restart, hiding the per-port failure while the global pending state survives. | Skip or preserve error rows while `pending_generation` is set during startup restore; add a restart-after-partial-apply test. |
...
12. **Open — 2026-08-13 full-code bug-hunt batch:** 33 findings recorded as
    ...
    with status `open`. Eight are P1: the deliberate TC parse-uncertainty
    fail-open and its two crafted-packet bypass shapes (`REVIEW-ACL-075`),
    the no-op `pull_data(0)` linearization (`REVIEW-ACL-076`), the Python 2.7
    unicode domain-key drop (`REVIEW-ACL-077`), false `ready` status after a
    failed port delete (`REVIEW-TXN-031`), the non-atomic `state.json` write
    (`REVIEW-TXN-032`), the WAL compact ordering window (`REVIEW-TXN-033`),
    and the config-parse fallback to standalone auto-attach
    (`REVIEW-OPS-038`). The remainder are P2/P3 error-path, idempotency,
    observability, and durability gaps. Batch by severity; none of these
    blocks P6 release promotion, but the P1 set must be reviewed against the
    RC before production activation.
