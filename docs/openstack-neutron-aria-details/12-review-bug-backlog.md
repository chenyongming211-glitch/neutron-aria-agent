...
| REVIEW-ACL-056 | P1 | Fragment-safe ACL/CT key semantics | implementation and hosted CI complete; privileged field evidence deferred | IPv4 non-first fragments are no longer parsed as ports and IPv6 uses the same bounded context contract. Guarded shared smoke wiring now requires exact pin-path/family public metric identities, destination-namespace receiver readiness, dual-direction dual-stack traffic, and bounded LRU pressure/eviction observation. | `FRAGMENT_TRACKING_SMOKE=1` is the explicit privileged execution gate. Standalone `MODE=system` remains single-interface; `MODE=tap` adds the authoritative cross-tap probe. Managed execution requires its explicit local peer netns/interface, dual-stack address, and VLAN contract. No privileged run was performed here: delivery is not field-verified, evidence remains deferred/pending, and shipped production configuration remains disabled. `update_failed` stays covered by Rust/eBPF hosted contracts because no safe deterministic LRU field trigger exists. |
