...
narrowed `REVIEW-TXN-036` (P2→P3), `REVIEW-TXN-037`, and `REVIEW-ACL-103`,
and withdrew none.
...
This register retains all 137 stable `REVIEW-*` IDs. Use the `Status` column,
...
| REVIEW-TXN-036 | P3 | ACL delete × durable partial transaction identity | open; narrowed by 2026-08-15 adversarial re-verification | `apply_delete_neutron_port` (`agent/src/neutron_api.rs:4690-4727`) has no pending-snapshot guard: while the runtime holds the durable retryable `partial` identity (`authority_state=partial`, `pending_generation=Some(G)`, the `REVIEW-ACL-080` retry contract), a port delete proceeds to `append_delete_intent` → purge → detach → `append_delete_commit(build_committed_delete_runtime(...))`. `build_committed_delete_runtime` (`neutron_api.rs:4433-4445`) sets `pending_generation=None` and `desired_hash=applied_desired_hash` but preserves `authority_state="partial"`, and WAL replay validation (`neutron_wal.rs:225-241`) accepts that commit because it requires exactly the pending=None state. The durable partial snapshot transaction identity is silently discarded, bypassing the single-unresolved-transaction invariant enforced at admission/prepare (`REVIEW-ACL-036`), and the committed runtime (`partial` with no pending) has no match in `project_neutron_status_v1` (`neutron_api.rs:1929-2069`), projecting `operator_blocked`. Adversarial re-verification confirmed the mechanism at every cited line but narrowed reachability and corrected the impact: the production Python pipeline is double-gated (`state.py:643-649` local pending check plus the `event_loop.py:2065-2096` remote action gate), so only direct UDS clients can trigger it; and the wedge is more persistent than first recorded — periodic full resync is itself blocked by the operator gate (`event_loop.py:278-281`), recover-pending is rejected with 409 because `pending=None`, and restart does not rebuild the accepted/applied identity. No enforcement impact was demonstrated; severity lowered P2 → P3. | Guard `apply_delete_neutron_port` against an unresolved durable partial (reject with `snapshot_apply_in_progress` semantics or complete recover-pending first), or make the delete commit preserve the pending identity and set a recoverable authority instead of leaving `"partial"` with no pending generation. |
| REVIEW-ACL-100 | P3 | Port-status projection latent false ready for acl-less ports | open; confirmed by 2026-08-15 adversarial re-verification | `acl_domain_status_for` (`agent/src/neutron_api.rs:4016-4019`) returns `domain_status("acl","ready",None)` when a port carries `managed_domains=["acl"]` but no `acl` snapshot payload, while the datapath gate is actually off (`acl_runtime_transition`, `acl_enabled=false`). The raw `port_statuses` row carries `status:"ready"` with `effective_action:None`, and the Python reporter default (`status_reporter.py:302-306`) writes `effective_action="enforce"` for any ready row without an action. Adversarial re-verification confirmed every cited line and added: Rust admission performs shape-only validation and does not reject acl-less acl-managed payloads; the production Python builder always attaches an acl block (`inventory.py:189-190` plus the `_load_acl_index` fallback), so the Python pipeline is unreachable, but a direct UDS client not only creates the latent `enforce` row — the same input also wedges the whole-machine Status V1 into `operator_blocked` (the `REVIEW-TXN-036` projection gap). The Python default at `302-306` was not touched by the `REVIEW-ACL-048` fix (overwrite path vs create-row-plus-default path). | Return `degraded/bypass` from `acl_domain_status_for` for acl-managed ports without an acl payload, or remove the Python ready-without-action → `enforce` default unless a concrete acl payload exists. Add a direct-UDS contract test. |
| REVIEW-ACL-101 | P3 | ACL runtime scrub swallows map iteration errors | open; confirmed by 2026-08-15 adversarial re-verification | All eight scrub helpers in `core/src/ebpf_ops/scrub.rs` (`scrub_hash_map` 76-80, `scrub_per_cpu_hash_map` 104-108, `scrub_lpm_v4_map` 120-127, `scrub_lpm_v6_map` 139-146, `scrub_policy_bank_map` 165-169, `scrub_rule_stats_bank_map` 186-190, `scrub_iface_ctx_entries` 201-205, `scrub_tap_config_entries` 220-224) collect keys with `iter().filter_map(|item| item.ok())`, silently dropping per-entry iteration errors; remove errors propagate but iteration errors do not, and there is no post-delete empty-set verification. Adversarial re-verification read the aya 0.13.1 iterator source and confirmed the impact is stronger than first recorded: the first non-ENOENT syscall error terminates iteration permanently, so every key after the error point is silently skipped, and a shadow-bank scrub that misses entries can activate residue in the new active bank for the same tap after the bank switch (tap-id reuse is real, `control_plane.rs:8336-8382`). The `validate_strict_pinned_runtime_state` fallback covers only the active bank and runs only at reconcile/admission points. | Propagate iteration errors (aggregate like `execute_map_delete_batch`) or add post-scrub `verify_empty` passes for the policy/bitmap/LPM families, mirroring `fragment_sweep_with`. |
| REVIEW-ACL-102 | P3 | CT contract stats read swallows iteration errors | open | `get_ct_contract_stats` (`core/src/ct_contract_ops.rs:41-58`) iterates the per-CPU `CT_CONTRACT_STATS` map with `if let Ok((key, values)) = item`, so per-entry iteration faults are skipped and the API returns HTTP 200 with silently incomplete CT-contract statistics. | Propagate iteration errors like `inventory.rs` `collect_runtime_network_entries`; keep empty and failed distinct. |
| REVIEW-TXN-037 | P3, conditional | WAL inventory read errors silently truncate the scan | open; narrowed by 2026-08-15 adversarial re-verification | `inventory_wal` (`core/src/wal.rs:189`) scans with `BufReader::new(file).lines().map_while(Result::ok)`, so a read I/O error (or a non-UTF-8 record, same failure class as the `REVIEW-OPS-027` repair) stops the scan silently: later checkpoint markers and mutations are not counted, and `WalWriter::open` (235-241) computes `next_checkpoint_id`/`header_required` from that truncated view. Adversarial re-verification corrected the original attribution: `inventory_wal` is called only by `WalWriter::open`; startup replay (`load_with_wal`, 792-845) rescans the file independently and never consumes the inventory view, so "replay rebuilds enforcement from an incomplete WAL" does not hold. The residual impact is (a) inventory read errors are completely unobservable — the `failures>0` accounting that `REVIEW-OPS-027` established applies only to the replay path, while standalone startup never checks `last_wal_replay_failures()` (`health.rs:23`); (b) an underestimated `next_checkpoint_id` can collide with an unseen marker in the persist-before-truncate crash window (TXN-033-class, very narrow); (c) underestimated `entry_count` can skip the startup compact. | Make `inventory_wal` propagate read errors and have `WalWriter::open` fail or record them; expose the failure count to `system_start` (call sites `control_plane.rs:5512`/`4958`). |
| REVIEW-ACL-103 | P3 | Conntrack key lacks the VLAN dimension that fragment context enforces | open; narrowed by 2026-08-15 adversarial re-verification | `CtKey4`/`CtKey6` (`abi/src/lib.rs:76-96`) carry tap/IP/ports/proto only, while `FragmentContextKey4/6` (`abi/src/fragment.rs:74-95`) include `vlan_id`; the parser does populate `info.vlan_id` from a single 0x8100 tag (`parser.rs:204-213`/`337-346`), and `ct_lookup_v4/v6` (`conntrack.rs:158-164`/`225-230`) update `last_seen`/counters/`SEEN_REPLY`/promotion per hit. Adversarial re-verification confirmed the mechanism but narrowed reachability: the triggering 802.1Q trunk tap is outside the supported deployment — the three target computes have no Neutron trunk extension (`docs/evidence/openstack-n05-lite/2026-06-30-discovery-summary.md:44`) and trunk/VLAN-subport port classes are explicitly `support_disposition=unsupported` (`docs/openstack-neutron-agent-mode.md:1811`, `docs/openstack-target-env-discovery.md:331`/`457-458`). Policy/QoS keys are equally VLAN-blind, so even a trunk tap would produce no wrong allow/drop today. Keep as pure isolation-boundary debt; escalate to an active defect only if VLAN-aware policy or trunk port-class support is introduced. | Add `vlan_id` to `CtKey4/6` (ABI change with layout tests) or disable cross-VLAN CT reuse on trunk taps. |
| REVIEW-ACL-104 | P3 | Port-filtered trace can never capture fragment resolve-stage drops | open | `FLAG_TRACING` is computed once in `load_feature_flags_tc` (`ebpf/src/lib.rs:529`) before fragment resolve, when non-first fragments already have `src_port/dst_port=0`; the resolve-stage drop branches (`lib.rs:181-184`/`385-388` → `phase_fragment_drop_tc` 710-716) never call `refresh_trace_flag_tc`, and `trace::should_trace` (`ebpf/src/trace.rs:44-49`) rejects any nonzero port filter when the packet ports are 0. All fragment-context drops therefore stay out of TRACE_LOG/TRACE_EVENTS for port-filtered tracing while passing fragments of the same flow are traced normally. | Refresh the trace flag in the resolve-drop phases using resolved/context ports, or document the port-0 semantics explicitly. |
| REVIEW-ACL-105 | P3 | IPv6 non-first fragment drops attributed to the extension-header protocol | open | The IPv6 parser stops extension-header traversal at a NonInitial fragment (`ebpf/src/parser.rs:416-418`) and sets `info.proto = next_header` (`parser.rs:462`), so Fragment→HOPOPTS/ROUTING/DSTOPTS→TCP chains keep proto 0/43/60. Resolve-stage drops occur before the context L4-proto overwrite (`ebpf/src/fragment.rs:142`/`157-159`/`204-209`/`222-224` vs `234-236`), so drop records and trace events are keyed under the wrong protocol and one logical flow's drops split across `DROP_REASON_STATS` proto keys. | Attribute the drop with the context L4 protocol when recoverable, or unify the on-wire semantics. |
| REVIEW-ACL-106 | P2 | ethertype case normalization gap degrades valid ACLs to bypass | open; confirmed by 2026-08-15 adversarial re-verification | The write path validates ethertype case-insensitively (`acl_contract.py:94-96`, `_text()` lowercases) and stores the client's original value (`"ipv4"`, `"IPV4"`, ...); the effective-ACL compile path compares the raw value against `_ip_version()` which always returns `"IPv4"` (`effective_acl.py:628-632`). Any non-canonical-case rule therefore compiles to `ethertype_cidr_mismatch`, the port degrades to `degraded/bypass`, and enforcement silently never engages (fail-open). direction/action/protocol all normalize at compile time; ethertype alone does not. Adversarial re-verification confirmed the full chain end-to-end (write → DB round-trip → compile, reproduced with python3) and confirmed there is no upstream compensation: Rust `normalize_acl_rule` (`agent/src/neutron_api.rs:5570-5577`) only rejects IPv6 and neither normalizes nor rejects lowercase `"ipv4"`. Severity stays P2: the `REVIEW-ACL-009` P1 precedent had a standard-client trigger, while this requires a client to send a non-canonical-cased value; escalate to P1 only with evidence that such inputs occur in practice. | Compare lowercase at compile time or canonicalize at write time in `prepare_rule`; add non-canonical-case unit tests. |
| REVIEW-ACL-107 | P3 | Port-status delete 404 permanently wedges status reporting and heartbeat | open; confirmed by 2026-08-15 adversarial re-verification | `remove_port_status` (`status_reporter.py:251-254`) adds the id to `pending_deleted_port_ids` and calls `_delete_one`; a deterministic 404 (row already absent: `delete_port_status` raises `AriaAclNotFound` when `rowcount==0`, `api.py:574-588`/`999-1011`, mapped to 404 in `exceptions.py:55-62`) raises before the `discard`, leaving the id pending forever. `report()` (238-249) runs `_flush_pending_deletes` first, so every cycle re-raises the same 404 before writing any status row. Adversarial re-verification confirmed the mechanism and added two precision corrections: (a) the heartbeat wedge applies only while the reporter is in ready non-degraded state — in degraded state the heartbeat is published before port rows (`status_reporter.py:335-358`); (b) the failure is per-cycle (`report_status` catches the error and returns `ok:False`, `event_loop.py:3610-3634`), the loop stays alive, and only a process restart clears the stuck pending id. Reachable when full-resync diff cleanup deletes a port whose row was never written or was already deleted (for example after `REVIEW-ACL-033` partial failure or post-migration); the unit tests only cover transient-failure retry, not 404. The consequence (Neutron sees the agent go down until restart) makes P2 also defensible; the fix is trivial. | Treat NotFound/404 as idempotent success (discard and continue); retain only transport/5xx errors for retry. |
| REVIEW-ACL-108 | P3 | EventMerger silence window starves event-driven convergence under sustained events | open | `EventMerger.ready()` (`event_merge.py:167-171`) requires `clock() >= last_pending_at + merge_interval`; with events arriving faster than the merge interval, `_last_pending_at` keeps advancing and `drain()` never runs (verified: 50 events at 0.1s with interval 0.2s → zero drains). The main loop then relies solely on periodic full resync (default 60s); the overflow path (207-217) sets `_full_resync` but drain remains blocked, and overflow clearing drops `_deleted_ports`. Bounded-latency liveness defect; no wrong allow/drop. | Add an absolute drain deadline or drain-on-overflow; preserve delete events across overflow. |
| REVIEW-ACL-109 | P3 | Address-set update always rewrites every member row | open | `update_address_set` (`db/aria_acl/api.py:848-865`) clones the existing row (which always contains members) into `current`, so `"members" in current` is always true and `_replace_members` (1349-1362) deletes and re-inserts every member with fresh ids/timestamps on every update — including name-only edits. Pure write amplification and row churn; correctness is preserved by the enclosing transaction. | Only call `_replace_members` when the update explicitly supplies `members` (`"members" in values`). |
| REVIEW-ACL-110 | P3, conditional | MySQL rowcount=0 false-negative turns idempotent status writes into HTTP 500 | open | `upsert_port_status` (`db/aria_acl/api.py:958-970`) treats `update.rowcount == 0` as "row absent": it inserts, catches IntegrityError, retries the update, and re-raises when the retry also reports 0. MySQL reports 0 affected rows when SET values are unchanged, and `updated_at` (DATETIME, second precision) can round-trip to the identical stored value for two writes in the same second, so an idempotent repeat write can raise a pseudo IntegrityError 500. Conditional: requires the target MySQL DATETIME truncation and a same-second double write (for example report-once interleaved with event paths). | Use an existence check (`SELECT 1`) instead of rowcount, or insert-first with savepoint absorption. |
| REVIEW-TXN-038 | P3, conditional | Python SnapshotStateStore._write lacks directory fsync after rename | open | `_write` (`agent/state.py:805-830`) does file fsync + `os.replace` but never fsyncs the parent directory, so the durable pending-snapshot identity can regress to the previous content across a power loss in the rename window. Rust `StateManager`/`WalWriter` already do directory fsync (`REVIEW-TXN-032`); this is the separate Python persistence layer. Conditional: impact is bounded by restart alignment against the Rust (generation, hash) identity. | Fsync the state directory after `_replace` on POSIX. |
...
consequences. `REVIEW-ACL-086` remains the target-kernel verification gate. The open
...
| `REVIEW-ACL-103` | narrowed | Recorded as pure isolation-boundary debt: trunk/VLAN-subport port classes are `support_disposition=unsupported` in the supported deployment; escalate only with VLAN-aware policy or trunk support. |
