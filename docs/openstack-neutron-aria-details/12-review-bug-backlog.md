...
| REVIEW-OPS-043 | P1 | Legacy Neutron HTTP requests have no independent timeout | source fixed; field retest pending | On the three-compute RC control-plane gate, a TCP peer that accepted the Keystone/Neutron connection but never returned an HTTP response blocked the legacy `python-neutronclient` call beyond the external 10-second probe. The installed client default was `timeout=None`, so a single stalled API request could stop full resync and heartbeat progress indefinitely. | Add the positive `[neutron] api_timeout` contract with a conservative `10.0s` default and pass it to the port source, ACL source, and port-status client. Keep this separate from the `3.0s` Aria UDS timeout and do not add another retry loop. Source RED/GREEN and all fast contracts pass; target Python 2.7 packaging, blackhole timeout, recovery, and three-node health evidence remain required before closure. |
