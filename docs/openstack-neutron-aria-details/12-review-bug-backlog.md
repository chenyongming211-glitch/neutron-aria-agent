...
| REVIEW-ACL-056 | P1 | Fragment-safe ACL/CT key semantics | implementation and hosted CI complete; privileged field evidence deferred | IPv4 non-first fragments are no longer parsed as ports and IPv6 uses the same bounded context contract. The guarded shared field driver emits valid IPv4/IPv6 UDP/53 fragments including 802.1Q, verifies delivery and public fragment metrics, and is wired into standalone and managed smoke entrypoints. | `FRAGMENT_TRACKING_SMOKE=1` is the explicit privileged execution gate. Standalone `MODE=system` covers the single-interface matrix; `MODE=tap` adds cross-tap isolation. Managed execution requires its explicit local peer netns/interface, dual-stack address, and VLAN contract. No privileged run was performed here: evidence remains deferred/pending and shipped production configuration remains disabled. |
