...
11. **Complete evidence and lower-risk hardening:** `REVIEW-ACL-007` is fixed
    ...
    This does not close `REVIEW-ACL-012`, whose clean target-container
    import/entrypoint evidence remains pending. `REVIEW-OPS-035` is also fixed:
    ...
    passed the executable contract plus the complete required build. Also close
