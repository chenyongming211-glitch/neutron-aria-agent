...
| REVIEW-ACL-046 | P1 | Cross-domain ACL selector isolation | implementation and hosted CI complete; privileged field evidence pending | Reopened 2026-07-15 with a complete enforcement path. The repair derives ACL maps from final direction-specific rule references, uses a conflict-aware general projection, gates ownership/skip on projection health, and repairs legacy pollution through bank publication plus strict CT invalidation while preserving standalone direct publication for `REVIEW-ACL-057`. The transaction implementation is `49081c6`. Pre-field wiring/hardening commits `d1aa523..ad30cad` cover managed detach ordering, purge-failure atomicity, strict-flush rollback, and successful retry detach; independent final review approved the wiring. Exact-head GitHub Actions run [29672271181](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29672271181) at `ad30cad` passed `fast-contracts`, `rust-behavior`, and `rust-build`. | Keep the finding not fixed and the PR Draft until privileged managed and standalone field evidence proves the completion conditions in `17-acl-selector-ownership-isolation.md`. No field execution is claimed by this checkpoint. |
...
| REVIEW-TXN-030 | P1 contract / P2 runtime | Strict CT flush managed-publication rollback | implementation and hosted CI complete; privileged field evidence pending | Commit `49081c6` moves strict CT flush inside the lifecycle/instance-locked owned publication transaction and restores bank, general-map, and durable preimages on failure while keeping health unverified when the primary operation or compensation fails. Pre-field wiring/hardening commits `d1aa523..ad30cad` cover managed detach ordering, purge-failure atomicity, strict-flush rollback, and successful retry detach; independent final review approved the wiring. Exact-head GitHub Actions run [29672271181](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29672271181) at `ad30cad` passed `fast-contracts`, `rust-behavior`, and `rust-build`. | Keep the finding not fixed until privileged field evidence proves strict-flush failure restores the old bank and general selectors, plus the final review gate completes. No field execution is claimed by this checkpoint. |
| REVIEW-ACL-057 | P1 | Direct ACL publication leaves same-bank CT valid | open; implementation batch includes `REVIEW-ACL-066` | Standalone/direct policy add, update, delete, and batch mutate the active ACL bank. Adding a CIDR to a standalone group already referenced by an ACL policy also mutates active ACL selector maps. CT validity is bank-based, so existing PASS decisions remain current after either change and active traffic can refresh them. | Use one full shadow-bank publication transaction for every direct/batch policy mutation and every ACL-referenced standalone group membership change. Strictly persist the final state, switch bank, invalidate CT, and restore kernel, bank, memory, allocator, and durable preimages on failure. Add live-flow add-deny, allow-to-deny, delete-allow, and referenced-deny-group expansion regressions. Keep ordinary unreferenced-group durability under `DEBT-ACL-001`. |
...
| REVIEW-ACL-064 | P1 | Detach purge active-policy-miss PASS window | implementation and hosted CI complete; privileged field evidence pending | Commit `49081c6` replaces item-at-a-time detach purge with the ordered transaction: quiesce ACL/CT, publish an empty owned ACL projection, strictly flush CT, then detach; purge or flush failure keeps the interface attached and quiesced. Pre-field wiring/hardening commits `d1aa523..ad30cad` cover managed detach ordering, purge-failure atomicity, strict-flush rollback, and successful retry detach; independent final review approved the wiring. Exact-head GitHub Actions run [29672271181](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29672271181) at `ad30cad` passed `fast-contracts`, `rust-behavior`, and `rust-build`. | Keep the finding not fixed until privileged managed field evidence proves no packet is accepted through policy-miss during detach preparation and retry behavior is correct. No field execution is claimed by this checkpoint. |
| REVIEW-ACL-065 | P1 | Privileged purge partial owned-ACL deletion | implementation and hosted CI complete; privileged field evidence pending | Commit `49081c6` removes privileged item deletion in favor of one quiesced empty-owned-ACL publication with complete rollback; failure aborts detach and preserves the complete prior owned state. This remains the root cause beneath the older ignored-error symptom in `REVIEW-ACL-023`. Pre-field wiring/hardening commits `d1aa523..ad30cad` cover managed detach ordering, purge-failure atomicity, strict-flush rollback, and successful retry detach; independent final review approved the wiring. Exact-head GitHub Actions run [29672271181](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29672271181) at `ad30cad` passed `fast-contracts`, `rust-behavior`, and `rust-build`. | Keep the finding not fixed until privileged managed field evidence proves purge failure leaves the interface attached/quiesced with complete old owned state and a successful retry detaches. No field execution is claimed by this checkpoint. |
| REVIEW-ACL-066 | P1 | Referenced standalone group expansion leaves same-bank CT valid | open; scheduled as a required subfinding of the `REVIEW-ACL-057` implementation batch | `add_group_standalone_locked` permits adding a CIDR to an existing group even when an ACL policy references that group, and writes the new selector directly into the active ACL source/destination bank. A previously cached PASS flow that should match a deny after the membership expansion keeps the same bank identity and can continue refreshing the stale decision. Whole-group delete is already rejected while a policy references the group, so the reproduced boundary is membership expansion rather than referenced deletion. | Route ACL-referenced standalone group membership changes through the same shadow-bank publication, strict persistence, CT invalidation, and rollback transaction used by `REVIEW-ACL-057`. Add a live-flow regression proving expansion into a deny group invalidates the old PASS. Do not close this ID until that group-specific evidence passes; keep ordinary unreferenced-group persistence under `DEBT-ACL-001`. |
...
4. **Field evidence pending — Isolate ACL selector ownership:**
   ...
   Privileged managed and standalone evidence is not available, so the finding
   remains not fixed and the PR remains Draft; no field execution is claimed.
5. **Unify direct ACL publication:** implement `REVIEW-ACL-057` together with
   its required referenced-group subfinding `REVIEW-ACL-066`, then complete
   `REVIEW-ACL-059`. Move standalone policy add/update/delete/batch and
   ACL-referenced group membership changes to one shadow-bank transaction.
   Prove existing allowed flows see a newly added deny, an allow-to-deny update,
   a deleted allow, and a referenced deny-group expansion without manual flush.
   Then quarantine bitmap indices until kernel cleanup is proven. Preserve both
   stable IDs and close each only with its own evidence.
