...
exact head after the quiesced-legacy-TC and parser-expansion commit series.
...
| REVIEW-TXN-036 | P2 | ACL delete × durable partial transaction identity | open | `apply_delete_neutron_port` (`agent/src/neutron_api.rs:4690-4727`) has no pending-snapshot guard: while the runtime holds the durable retryable `partial` identity (`authority_state=partial`, `pending_generation=Some(G)`, the `REVIEW-ACL-080` retry contract), a port delete proceeds to `append_delete_intent` → purge → detach → `append_delete_commit(build_committed_delete_runtime(...))`. `build_committed_delete_runtime` (`neutron_api.rs:4433-4445`) sets `pending_generation=None` and `desired_hash=applied_desired_hash` but preserves `authority_state="partial"`, and WAL replay validation (`neutron_wal.rs:225-241`) accepts that commit because it requires exactly the pending=None state. The durable partial snapshot transaction identity is silently discarded, bypassing the single-unresolved-transaction invariant enforced at admission/prepare (`REVIEW-ACL-036`), and the committed runtime (`partial` with no pending) has no match in `project_neutron_status_v1` (`neutron_api.rs:2024-2058`), projecting `operator_blocked` with no self-heal until the next full resync. Replay stays clean, so no corruption; no wrong allow/drop demonstrated. | Guard `apply_delete_neutron_port` against an unresolved durable partial (reject with `snapshot_apply_in_progress` semantics or complete recover-pending first), or make the delete commit preserve the pending identity and set a recoverable authority instead of leaving `"partial"` with no pending generation. |
| REVIEW-ACL-100 | P3 | Port-status projection latent false ready for acl-less ports | open | `acl_domain_status_for` (`agent/src/neutron_api.rs:4016-4019`) returns `domain_status("acl","ready",None)` when a port carries `managed_domains=["acl"]` but no `acl` snapshot payload, while the datapath gate is actually off (`acl_runtime_transition`, `acl_enabled=false`). The raw `port_statuses` row carries `status:"ready"` with `effective_action:None`, and the Python reporter default (`status_reporter.py:302-306`) writes `effective_action="enforce"` for any ready row without an action. The Rust Status V1 projection itself blocks that row and the current Python snapshot builder always attaches an acl block for acl-managed ports, so the combination is unreachable through the production pipeline today; it is reachable by direct UDS consumers of raw `port_statuses`. | Return `degraded/bypass` from `acl_domain_status_for` for acl-managed ports without an acl payload, or remove the Python ready-without-action → `enforce` default unless a concrete acl payload exists. Add a direct-UDS contract test. |
| REVIEW-ACL-101 | P3 | ACL runtime scrub swallows map iteration errors | open | All eight scrub helpers in `core/src/ebpf_ops/scrub.rs` (`scrub_hash_map` 76-80, `scrub_per_cpu_hash_map` 104-108, `scrub_lpm_v4_map` 120-127, `scrub_lpm_v6_map` 139-146, `scrub_policy_bank_map` 165-169, `scrub_rule_stats_bank_map` 186-190, `scrub_iface_ctx_entries` 201-205, `scrub_tap_config_entries` 220-224) collect keys with `iter().filter_map(|item| item.ok())`, silently dropping per-entry iteration errors; remove errors propagate but iteration errors do not, and there is no post-delete empty-set verification. A transient `bpf_map_get_next_key` fault during scrub leaves tap-scoped POLICY / PORT_BITMAP_POOL / ACL-LPM entries behind that are indistinguishable from legitimate entries after tap-id reuse; the later strict pinned-runtime inventory validation would catch the drift as degraded (defense in depth). | Propagate iteration errors (aggregate like `execute_map_delete_batch`) or add post-scrub `verify_empty` passes for the policy/bitmap/LPM families, mirroring `fragment_sweep_with`. |
...
| REVIEW-TXN-037 | P3, conditional | WAL inventory read errors silently truncate the scan | open | `inventory_wal` (`core/src/wal.rs:189`) scans with `BufReader::new(file).lines().map_while(Result::ok)`, so a genuine read I/O error stops the scan silently: later checkpoint markers and mutations are not counted, `next_checkpoint_id`/`header_required` may be computed from a truncated view, and startup replay can rebuild enforcement from an incomplete WAL while only a health counter observes it. `load_with_wal` itself counts read failures (839-843), so the replay path is bounded; the inventory path is not. Conditional: not yet confirmed whether standalone startup tolerating this is intentional best-effort design; if it is, the remaining gap is observability. | Make `inventory_wal` propagate read errors and have `WalWriter::open` fail or record them; expose the failure count to `system_start`. |
| REVIEW-ACL-103 | P3 | Conntrack key lacks the VLAN dimension that fragment context enforces | open | `CtKey4`/`CtKey6` (`abi/src/lib.rs:76-96`) carry tap/IP/ports/proto only, while `FragmentContextKey4/6` (`abi/src/fragment.rs:74-95`) include `vlan_id`. On a tap carrying 802.1Q trunks, flows with identical five-tuples in different VLANs share one CT entry: `ct_lookup_v4/v6` fast-paths both flows, and `last_seen`/`pkt_count`/`byte_count`/`SEEN_REPLY`/NEW→ESTABLISHED promotion are cross-polluted. Current policy/QoS keys are equally VLAN-blind, so no wrong allow/drop occurs today, but the isolation boundary is inconsistent and any future VLAN-aware policy would turn this into wrong enforcement. | Add `vlan_id` to `CtKey4/6` (ABI change with layout tests) or disable cross-VLAN CT reuse on trunk taps. |
...
| REVIEW-ACL-106 | P2 | ethertype case normalization gap degrades valid ACLs to bypass | open | The write path validates ethertype case-insensitively (`acl_contract.py:94-96`, `_text()` lowercases) and stores the client's original value (`"ipv4"`, `"IPV4"`, ...); the effective-ACL compile path compares the raw value against `_ip_version()` which always returns `"IPv4"` (`effective_acl.py:628-632`). Any non-canonical-case rule therefore compiles to `ethertype_cidr_mismatch`, the port degrades to `degraded/bypass`, and enforcement silently never engages (fail-open). direction/action/protocol all normalize at compile time; ethertype alone does not. P1 escalation is justified by the `REVIEW-ACL-009` precedent (valid-looking policy → degraded/bypass). | Compare lowercase at compile time or canonicalize at write time in `prepare_rule`; add non-canonical-case unit tests. |
| REVIEW-ACL-107 | P3 | Port-status delete 404 permanently wedges status reporting and heartbeat | open | `remove_port_status` (`status_reporter.py:251-254`) adds the id to `pending_deleted_port_ids` and calls `_delete_one`; a deterministic 404 (row already absent: `delete_port_status` raises `AriaAclNotFound` when `rowcount==0`, `api.py:574-586`/`999-1008`) raises before the `discard`, leaving the id pending forever. `report()` (238-249) runs `_flush_pending_deletes` first, so every cycle re-raises the same 404 before writing any status row or heartbeat. Reachable when full-resync diff cleanup deletes a port whose row was never written or was already deleted (for example after `REVIEW-ACL-033` partial failure or post-migration). | Treat NotFound/404 as idempotent success (discard and continue); retain only transport/5xx errors for retry. |
