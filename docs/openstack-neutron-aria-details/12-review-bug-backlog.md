...
pending. Fragment-safe ACL/CT implementation and hosted CI are complete, while
its privileged field evidence remains pending; the independent XDP hook-health
...
| REVIEW-ACL-056 | P1 | Fragment-safe ACL/CT key semantics | implementation and hosted CI complete; privileged field evidence deferred | IPv4 non-first fragments are no longer parsed as ports and IPv6 uses the same bounded context contract. First fragments publish bounded address/protocol/ID/tap/direction/VLAN/bank/epoch context before CT creation; later fragments recover the authoritative ports or fail closed. Final review also closed replacement-CT deletion (`REVIEW-ACL-067`) and stale per-CPU identity attribution (`REVIEW-ACL-068`). The guarded field orchestration installs direction-specific UDP/53 policy, covers dual-stack/direction/order, VLAN/tap isolation, publication stale, restart scrub, cleanup, and standalone capacity-8 pressure/oldest-identity eviction. | Parser/context RED Builds [29694226731](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29694226731) and [29694579684](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29694579684) led through Tasks 2-7 to field-wiring head `e4bac0c`, whose Build [29941244612](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29941244612) passed `fast-contracts`, `rust-behavior`, and `rust-build`. Final-review RED `dd35fb2` failed only on the intended missing ordering/reset contracts in Build [29942509813](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29942509813); GREEN `43e0e2a` passed all three hosted jobs in Build [29943612716](https://github.com/chenyongming211-glitch/aria-firewall/actions/runs/29943612716). `FRAGMENT_TRACKING_SMOKE=1` remains the privileged gate. No privileged run was performed: field evidence is `deferred/pending`, shipped production configuration remains disabled at capacity 8192, managed smoke preserves deployed capacity, and `update_failed` remains hosted-only because no safe deterministic LRU field trigger exists. |
