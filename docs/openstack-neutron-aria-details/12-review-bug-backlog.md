...
| REVIEW-TXN-028 | P1 | Post-apply status false commit | open | `_status_after_apply` returns `None` when status is unavailable, while full/scoped callers continue projection commit, clear pending, and `mark_ready`. Generation/hash convergence also ignores degraded authority/domain outcomes. Explicit non-converged pending polling is not affected. | Require a verified ready terminal status before committing. Status failure or degraded/blocked domain must retain pending and produce degraded/blocked agent state. Add full/scoped status-error and degraded-domain tests. |
| REVIEW-TXN-029 | P1 | Non-authoritative OVS inventory false commit | open | OVSDB failure becomes all-ignored or an empty non-authoritative plan. Because only `status=error` blocks commit, Rust can publish a new applied generation/hash and ready state without executing the intended attach/ACL change. | Treat inventory authority failure as retriable transaction failure while preserving old ports and avoiding detach. Do not advance applied/hash, clear pending, or replace prior statuses until inventory is authoritative. |
