...
| REVIEW-ACL-093 | P3 | delete_trace_filter treats read errors as not-present | open; design complete | `delete_trace_filter` converts every pre-read error to `Ok(false)`, suppressing removal and reporting false absence. The selected repair removes the pre-read and classifies the delete result directly: only a missing key is idempotent absence; operational faults propagate. | Formal ACL-only design: `docs/superpowers/specs/2026-08-15-review-acl-093-094-observability-delete-semantics-design.md`. RED/GREEN pending. |
| REVIEW-ACL-094 | P3 | ACL-facing batch flush helpers swallow iteration/removal failures | open; ACL-only scope designed | Trace logs, per-tap `DROP_REASON_STATS`, and ACL rule/group-stat cleanup count candidates or return success while discarding iterator/removal errors. The selected batch strictly enumerates candidates, attempts every collected deletion, counts only actual removals, and aggregates real failures. Global kernel-drop, QoS, Mirror and TCP-RT paths are explicitly excluded and remain recorded non-ACL debt. | Formal ACL-only design: `docs/superpowers/specs/2026-08-15-review-acl-093-094-observability-delete-semantics-design.md`. RED/GREEN pending; no datapath or privileged evidence applies. |
