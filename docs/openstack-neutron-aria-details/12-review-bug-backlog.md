...
Full re-check of all recorded `REVIEW-*` IDs against the current tree. This is
a classification-only refresh; no runtime code fixes are included. The
...
| Verdict | Count | IDs |
| --- | ---: | --- |
| Confirmed active defect or contract gap | 54 | All active register rows except the six items classified below |
| Fixed | 2 | `REVIEW-ACL-016`, `REVIEW-ACL-018` |
...
The 54 active items are grouped by failure surface so that runtime bugs are not
mixed with delivery and documentation gaps:

| Active class | Count | IDs |
| --- | ---: | --- |
| Transaction, datapath, recovery, and runtime consistency | 27 | `ACL-023`, `ACL-025`, `ACL-026`, `ACL-028`, `ACL-033`, `ACL-035`-`ACL-037`, `ACL-039`, `ACL-044`, `ACL-045`, `ACL-047`-`ACL-051`, `ACL-053`, `ACL-054`; `TXN-021`, `TXN-022`, `TXN-024`-`TXN-027`; `OPS-019`, `OPS-027`, `OPS-034` |
| Northbound API, DB, compile, and status projection correctness | 16 | `ACL-001`-`ACL-004`, `ACL-006`, `ACL-008`, `ACL-009`, `ACL-013`, `ACL-029`-`ACL-031`, `ACL-038`, `ACL-040`-`ACL-043` |
| Packaging, deployment, validation, documentation, and release gaps | 11 | `ACL-005`, `ACL-007`, `ACL-010`, `ACL-011`, `ACL-014`, `ACL-015`, `ACL-017`; `DOC-020`, `DOC-021`; `OPS-035`; `CI-001` |
| **Total active defect or gap** | **54** | |

P1 set re-confirmed in source: `TXN-021`, `TXN-022`, `TXN-025`, `ACL-001`,
`ACL-009`, `OPS-019`, `ACL-035`, `ACL-039`, `ACL-048`, `ACL-049`, `ACL-053`.
...
| --- | ---: | --- |
| Active defect or contract gap | 54 | Open `REVIEW-*` register rows |
...
| REVIEW-ACL-029 | P2 | Empty address-set compiles as ready | open | `EffectiveAclIndex._compile_address_match` accepts address sets whose members list is empty (or only empty addresses) and returns no compile error, so rules can be `ACL_READY`/`enforce` with empty CIDR lists. | Treat empty member sets as degraded/unsupported before submit; cover with effective-ACL unit tests. |
| REVIEW-ACL-030 | P2 | Disabled address-set still expanded | open | `_compile_address_match` checks missing address-set IDs but never `_enabled(address_set)`. Disabled sets still expand members into effective rules. | Reject or degrade rules that reference disabled address sets; add unit tests. |
| REVIEW-ACL-031 | P2 | Effective-for-port API hardcodes eligible | open | `AriaAclPlugin.get_aria_acl_effective_for_port` always calls `effective_for_port(..., {"eligible": True})`, so non-OVS/non-compute ports can appear ready/enforce from the API even when the agent would mark them unsupported/bypass. | Pass real eligibility (or document API as desired-state-only and return an explicit disposition field). Add plugin tests for ineligible ports. |
...
| REVIEW-ACL-043 | P3 | priority=0 rejected as missing | open | `_require()` uses falsy `not obj.get(field)`, so rule `priority=0` fails validation while effective compile accepts 0. | Use explicit missing checks for numeric fields; add create/update unit tests for priority 0. |
...
| REVIEW-ACL-048 | P1 | Status projection overwrites bypass→enforce | open | `_port_statuses_from_status` replaces UDS `effective_action` values of `bypass` (and empty) with snapshot metadata defaulting to `enforce` when `acl_enabled` is true. Northbound `aria_acl_port_statuses` can report enforce while datapath bypassed. | Never overwrite a concrete UDS runtime `effective_action`/`status`; treat UDS as runtime truth. Add unit tests for UDS bypass + snapshot enforce. |
| REVIEW-ACL-049 | P1 | Unwired managed_domains wedge ACL | open | Config allows `qos`/`mirror` in `managed_domains`. Rust `reconcile_neutron_domains` treats any domain outside `attach|acl` as unimplemented and marks ACL `blocked` for the whole port. Distinct from `REVIEW-ACL-039` (missing qos payload). | Reject unwired domains at config validation, or implement them; never block ACL solely because another domain is unimplemented. |
| REVIEW-ACL-050 | P2 | ACL enforce without conntrack foundation check | open | Neutron ACL reconcile enables ACL via `update_config(..., acl=Some(true), ...)` without verifying conntrack. Loopback config can disable conntrack when `conntrack` is not in `managed_domains`. | Require CT ready before ACL enforce, or gate CT toggles on Neutron-managed ports; degrade ACL when CT unavailable for stateful policies. |
| REVIEW-ACL-051 | P2 | WAL recovery false-passes qos/mirror | open | Pending-intent recovery marks `qos`/`mirror` as `recovered` with `*_no_runtime_executor` and can still return `ok: true` when those domains were in the intent. | Treat unimplemented recovery domains as degraded/failed; do not report recovered success without an executor/scrub. |
...
| REVIEW-ACL-053 | P1 | Lenient ct_flush hides CT clear failure | open | `core::ct_ops::ct_flush` returns `Ok(0)` when CT map pins fail to open/convert. Neutron ACL reconcile treats that as success. XDP established CT can continue bypassing policy after ACL changes. Strict scrub exists but is unused here. | Use strict CT scrub for Neutron ACL reconcile, or fail reconcile when CT cannot be cleared. Add missing-pin fault tests. |
| REVIEW-ACL-054 | P2 | stateful=false still uses XDP CT fast-path | open | `NeutronAclSnapshot.stateful` is carried by DTO/Python but ignored by Rust translate/reconcile. `stateful=false` policies still create/use XDP CT fast-path. | Honor `stateful` in reconcile (disable CT create/fast-path for false; degrade when true and CT unavailable). Add translator/reconcile contract tests. |
| REVIEW-DOC-021 | P2 | Capabilities advertise unimplemented domains | open | `NEUTRON_SUPPORTED_DOMAINS` / `neutron-uds-contract.json` / capabilities response list qos/mirror/config/ct/… while reconcile only implements `attach`+`acl`. Stage-1 CI even requires qos/mirror in supported_domains. | Split advertised vs implemented domains; shrink supported set or mark planned and reject managed_domains that are unimplemented. |
...
4. `REVIEW-ACL-048` / `REVIEW-ACL-049` / `REVIEW-ACL-039`: stop false enforce and managed_domains wedges.
5. `REVIEW-OPS-019`: bound Neutron WAL growth and restart replay cost.
6. `REVIEW-ACL-001` / `REVIEW-ACL-009` / `REVIEW-ACL-029` / `REVIEW-ACL-030`: northbound false-ready contract.
7. `REVIEW-ACL-054` / `REVIEW-ACL-050` / `REVIEW-ACL-047`: stateful/CT/priority datapath contract.
8. `REVIEW-ACL-025` / `REVIEW-ACL-026` / `REVIEW-ACL-044`: owned-ACL durable ordering and no-op bank flips.
9. `REVIEW-ACL-023` / `REVIEW-TXN-024` / `REVIEW-TXN-027` / `REVIEW-ACL-045`: detach/delete/orphan convergence.
10. `REVIEW-ACL-036` / `REVIEW-ACL-037` / `REVIEW-ACL-028` / `REVIEW-ACL-008` / `REVIEW-ACL-033`: Python pending/status consistency.
11. `REVIEW-TXN-026`: gate accept until startup recovery completes.
12. `REVIEW-DOC-021`: shrink or phase-mark capabilities to implemented domains.
13. `REVIEW-ACL-038` / `REVIEW-ACL-040` / `REVIEW-ACL-041` / `REVIEW-ACL-042` / `REVIEW-ACL-043`: client/DB/CLI correctness.
14. `REVIEW-ACL-051`: reject recovery success for domains without a runtime executor.
15. `REVIEW-ACL-007`: first-install package rollback hygiene.
16. `REVIEW-ACL-002` / `REVIEW-ACL-006` / `REVIEW-ACL-003`: desired-state validation and HTTP mapping.
17. `REVIEW-ACL-031`: effective API eligibility.
18. `REVIEW-OPS-027` / `REVIEW-OPS-034` / `REVIEW-OPS-035` / `REVIEW-CI-001`: ops/CI hardening.
19. `REVIEW-ACL-010` / `REVIEW-ACL-013` / `REVIEW-ACL-015` / `REVIEW-ACL-017`: smoke/projection/rollback polish.
20. `REVIEW-DOC-020`: align domain-status detail documentation with current DTOs.
21. `REVIEW-ACL-011` / `REVIEW-ACL-014` / `REVIEW-ACL-005`: release hygiene and coverage.
