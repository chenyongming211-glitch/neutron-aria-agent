...
| REVIEW-ACL-046 | P1 | Cross-domain ACL selector isolation | design complete; implementation pending | Reopened 2026-07-15 with a complete enforcement path. Non-`neutron:*` local groups are allowed on an ACL-managed tap and are written to active ACL LPM banks; shadow staging also replays all groups. An exact or more-specific local CIDR returns the local group ID instead of the Neutron selector ID, so selector-specific deny can miss and default PASS. The allocator itself remains globally unique. | Implement `17-acl-selector-ownership-isolation.md`: derive ACL maps from final direction-specific rule references, use conflict-aware general projection so ACL-only data cannot shadow a local general identity, gate ownership promotion and snapshot skip on verified projection health, repair legacy pollution through bank switch plus strict CT flush, and preserve standalone direct publication for `REVIEW-ACL-057`. |
...
| REVIEW-TXN-030 | P1 contract / P2 runtime | Strict CT flush is outside managed ACL publication rollback | confirmed; repair planned | `replace_owned_acl` can apply the general delta, switch the active ACL bank, and persist final state before `reconcile_neutron_acl` performs the strict CT flush. Flush failure only re-quiesces the runtime gate; it cannot restore the previous active bank, general-map preimages, or durable state. The immediate behavior is fail-closed, but it violates the approved atomic publication contract and leaves the failed Neutron generation represented by newly committed runtime/durable state. | Move strict CT flush inside the lifecycle/instance-locked owned publication transaction before post-commit cleanup. Retain and reverse the existing bank/general receipts, restore the old durable state, keep health unverified on any primary or compensation failure, and add behavior-level failure-injection coverage. |
...
| REVIEW-ACL-064 | P1 | Detach purge creates an active-policy-miss PASS window | confirmed; repair planned | Snapshot detach calls `purge_neutron_acl` before `TapRegistry::detach`. The purge deletes rules from the current active ACL bank without first quiescing ACL/CT. Until TC links detach, a packet that no longer finds a policy entry follows the existing policy-miss `XDP_PASS` path, which TC maps to `TC_ACT_OK`. | Replace item-at-a-time purge with an ordered transaction: quiesce ACL/CT, publish an empty owned ACL projection, strictly flush CT, then detach. A purge or flush failure must keep the interface attached and quiesced. Add ordered behavior tests and a privileged live-packet regression. |
| REVIEW-ACL-065 | P1 | Privileged purge can persist a partial owned-ACL deletion | confirmed; repair planned | `purge_neutron_acl` deletes policies before groups. `delete_policy_for_neutron_purge` bypasses normal authority/health admission, while `delete_group_for_neutron_purge` delegates into managed local projection admission and rejects `Unverified`/`RepairRequired`. Failure cleanup logs the purge error and continues to detach; unregister compacts the partially mutated state before runtime scrub, so deleted policies and residual `neutron:{port_id}:*` groups can be durably split and reloaded on interface-name reuse. This is the root cause beneath the older ignored-error symptom in `REVIEW-ACL-023`. | Remove privileged item-delete purge. Use one quiesced empty-owned-ACL publication with complete rollback, abort detach on failure, and keep the prior owned state complete. Add purge-failure coverage asserting no detach, no partial durable state, and a successful retry. |
...
4. **In progress — Isolate ACL selector ownership:** `REVIEW-ACL-046` has a
   complete, independently reviewed implementation design in
   `17-acl-selector-ownership-isolation.md`; RED and production code are still
   pending. Derive separate managed general/ACL projections from final
   references, gate ownership/skip on projection health, repair legacy
   pollution through bank publication plus strict CT invalidation, and prove
