...
| REVIEW-ACL-076 | P1 | pull_data(0) never extends the linear head for non-linear skbs | open | `ebpf/src/lib.rs:546` and `:563` call `pull_data(0)`. In the Linux implementation len=0 is converted to `skb_headlen(skb)`, so the call at best ensures the existing linear head is writable (unclone) and is not literally a no-op — but it never pulls paged data and `pskb_may_pull` succeeds without extending the head. The intended non-linear-skb repair path (comment at `lib.rs:558-561`) is therefore ineffective for GRO/segmented traffic whose L4 header is paged, which then falls into the fail-open parse path recorded by `REVIEW-ACL-075`. The target-kernel (4.18 backport) semantics of the len=0 conversion should be confirmed when fixing. | Pull the bounded length needed to parse the L4 header (IP header plus transport header; do not default to `ctx.len()`, which would force full linearization and extra copies on large GSO skbs), then re-parse; add a non-linear-skb contract test. |
...
| REVIEW-TXN-032 | P1 | state.json non-atomic write may lose all persisted rules on a crash in the window | open | `StateManager::with_state` (`core/src/state.rs:688-701`) truncates and rewrites `state.json` in place with no tmp+rename; the WAL compactor uses the correct atomic pattern. A crash in that specific window can leave an empty or torn snapshot; if the WAL has already been compacted, `load_with_wal` (`core/src/wal.rs:609-623`) then starts from `FirewallState::default()` and replays nothing, so all persisted group/policy/QoS/mirror rules are lost and `evaluate_policy` defaults to pass. This is a crash-window possibility, not the outcome of every crash. | Persist state via tmp+fsync+rename+dir-sync like `WalWriter::compact`; add a torn-write recovery test. |
| REVIEW-TXN-033 | P2 | WAL compact snapshot-before-truncate window drifts the bitmap allocator | open | `WalWriter::compact` (`core/src/wal.rs:200-236`) renames the new snapshot before truncating the WAL. A crash between the two replays the full WAL prefix on top of the new snapshot; `apply_add_rule` (`core/src/state.rs:462-512`) is not idempotent for updates, so replayed older `AddRule` entries release and re-allocate port sets and can move the surviving rule's `bitmap_idx` (verified for a K:A then K:B then K:C chain). The proven impact is allocator drift: bitmap_idx movement and free-list/allocator divergence from the checkpoint, followed by startup re-projection of kernel maps from the drifted state. No wrong allow/drop, bitmap conflict, or permanent enforcement divergence has been demonstrated, so this is recorded P2 with a defined escalation path. | Do NOT reorder to truncate-first: that opens a data-loss window between truncate and snapshot rename. Correct directions: a checkpoint/epoch boundary so replay skips entries already reflected in the snapshot, or make rule replay strictly idempotent. Add a replay-parity test asserting allocator state after snapshot+full-WAL replay equals the checkpoint state; escalate back to P1 only if that test proves capacity exhaustion, index conflict, or incorrect enforcement. |
...
12. **Open — 2026-08-13 full-code bug-hunt batch:** 33 findings recorded as
    ...
    six P1 (`REVIEW-ACL-075/076/077`, `REVIEW-TXN-031/032`,
    `REVIEW-OPS-038`) and `REVIEW-TXN-033` at P2 with an escalation path;
    ...
    `REVIEW-ACL-083/084` and `REVIEW-TXN-035` marked conditional, and the
