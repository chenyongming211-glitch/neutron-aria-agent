...
8. **In progress — close transaction and recovery debt:**
   ...
   with `REVIEW-ACL-065`. Continue with `REVIEW-TXN-024`, then
   `REVIEW-TXN-027`, followed by the independent `REVIEW-ACL-045` full orphan
   runtime scrub. Do not fold these three remaining production defects into
