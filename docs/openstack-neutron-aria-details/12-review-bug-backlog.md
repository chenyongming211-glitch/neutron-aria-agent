...
8. **In progress — close transaction and recovery debt:**
   ...
   with `REVIEW-ACL-065`. `REVIEW-TXN-024` is now fixed in `95c440a`;
   continue with `REVIEW-TXN-027`, followed by the independent
   `REVIEW-ACL-045` full orphan runtime scrub. Do not fold the remaining
   production defects into one implementation commit.
