...
`REVIEW-ACL-098/099`, are fixed. `REVIEW-TXN-035` is closed after its combined
restart projection stayed `blocked/recover_pending`. `REVIEW-ACL-086` and
`REVIEW-ACL-083/084` remain verification gates. The open
...
12. **In progress — 2026-08-13 full-code bug-hunt batch:** 33 findings recorded as
    ...
    `REVIEW-ACL-083/084` remain conditional, `REVIEW-TXN-035` is closed after
    its combined restart projection disproved the false-ready consequence, and
