...
| OVS agent / ovs-vswitchd / ovsdb-server 重启行为 | 重启服务并记录 tap/ifindex/XDP 和 VM forwarding 变化 | tap 仍存在且 XDP/map 健康时 ACL attach 可保持 ready；tap 消失或 ifindex 改变时按 tap recreate 恢复；VM ping 单独作为 OVS forwarding 证据 | 2026-07-01 已执行一次 `ovs-vswitchd` 探针，tap/XDP 仍在但即时 ping 失败，cleanup 后恢复 | `docs/evidence/openstack-n05-lite/20260701-stage3-ovs-restart-probe/summary.md` | 重跑 ACL-focused ovs-restart smoke，拆分 ACL attach 与 OVS forwarding 结论 |
