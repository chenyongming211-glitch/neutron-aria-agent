...
- [x] Rename the allocation variables so creation order is unambiguous.
- [x] Assert the first allocation has the smaller id.
- [x] Keep the production `counters_groups` numeric sort and expected CIDR ordering unchanged.
- [x] Commit and push the test-only repair.
...
- [x] Preserve the disappeared-interface test identity.
- [x] Seed the persisted marker directly with the orphan ifname, a historical nonzero ifindex, and an inactive marker.
- [x] Do not call the live-interface reservation path for the disappeared interface.
- [x] Keep the assertion that link-only cleanup does not consume the retry marker.
- [x] Commit and push the test-only repair.

## Task 3: Close DEBT-CI-004 shell correctness baseline

**Files:**
- Modify only the 26 scripts named by Build 31890013101.

- [x] Correct SC2097/SC2098 environment assignment order so the invoked process receives the intended values and expansions see the intended preimage.
- [x] Correct SC1010, SC2155, SC2295, SC2196, and SC2029 with explicit syntax, status-preserving assignment, quoted parameter operations, `grep -E`, and deliberate local/remote expansion.
- [x] Quote SC2086 sites unless intentional argument splitting is part of the command contract; document and annotate only those intentional sites.
- [x] Replace ambiguous SC2015 chains with explicit `if`/`else` where failure behavior matters; annotate only literal boolean projections proven safe.
- [x] Preserve literal embedded shell/AWK/Python expressions at SC2016 sites with line-local explanations rather than changing their runtime expansion layer.
- [x] Rename intentionally unused retry counters/loop variables or annotate the narrow line; remove genuinely dead assignments.
- [x] Run `bash -n` over every modified shell script and the existing Cargo-free CI contract tests locally.
- [x] Commit and push the shell remediation.

## Task 4: Obtain hosted GREEN and close the four findings

Before the GREEN run, close `DEBT-CI-005` without a lint suppression:

- [x] Add a public, copyable `FragmentResolveInput` in `abi/src/fragment.rs` containing `tap_id`, `is_ipv6`, `active_bank`, `now_ns`, and `fragment_offset`.
- [x] Change `fragment_resolve_decision` to accept that input plus the existing config, epoch, and context references.
- [x] Update all eBPF and ABI-test callers without changing decision values, ABI map layouts, or datapath ordering.
- [x] Commit and push the API-structure remediation.

- [x] Dispatch `run_quality_audit=true` on the exact remediation head.
- [x] If new diagnostics appear, preserve the command/rules and repair them under this plan.
- [ ] Require the full host-workspace test, clippy, rustfmt, Ruff, and ShellCheck steps all green.
- [x] Update `DEBT-CI-002` through `DEBT-CI-005` to fixed with exact commit/run evidence.
