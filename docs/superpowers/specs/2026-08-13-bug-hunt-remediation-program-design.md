...
| `REVIEW-ACL-094` | conflict | `ERROR-EXACT`, `OBS-EXACT` | The ACL-facing batch flushes report only actual removals and aggregate every per-key failure; requested-key count is never returned as deletion count. Global kernel-drop, QoS, Mirror and TCP-RT paths remain excluded non-ACL debt. | Multi-key partial-failure tests across trace, drop-reason and ACL rule/group statistics helpers. |
