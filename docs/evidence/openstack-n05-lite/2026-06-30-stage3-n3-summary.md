...
| S3-3 no-binding | fault | pending | Stage-two source/full-resync evidence covers normal bound ACL input. | Need explicit no-binding proof that the port remains bypass/not_requested and local ACL authority is not claimed. | Run a no-binding full-resync probe and capture status/API output. |
