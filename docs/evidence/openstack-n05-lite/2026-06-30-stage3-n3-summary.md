...
| S3-4 ovs-restart | lifecycle | degraded | `docs/evidence/openstack-n05-lite/20260701-stage3-ovs-restart-probe/summary.md` | Controlled `ovs-vswitchd.service` restart on `ostack2.bj159.net` exposed a lifecycle convergence gap: full-resync generation 145 put five ports under ACL management and the target VM ping passed before restart, but the immediate post-restart ping failed with 100% loss even though the tap and XDP attachment were still present. Cleanup deleted all managed ACL ports, UDS returned to `ready` with zero managed ports, and VM connectivity recovered. | Define and validate the production OVS restart/recovery sequence, such as ordered OVS agent restart plus forced full-resync, before marking this gate pass. |
