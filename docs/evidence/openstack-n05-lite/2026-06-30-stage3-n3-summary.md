...
| S3-3 no-binding | fault | degraded | `docs/evidence/openstack-n05-lite/20260630-stage3-no-binding-probe/summary.md` | Probe confirmed no `aria_acl` bindings and successful cleanup/rollback, but the deployed binary reported ACL domain `ready` instead of `not_requested` / `bypass`, and one-shot agent returned non-zero while datapath converged. Local ACL authority may be claimed when the ACL domain is Neutron-managed. | Deploy status-fidelity fix and rerun this probe to move disposition to `pass`. |
| S3-3 missing-policy | fault | pending | `deploy/kolla/smoke/neutron_aria_acl_fault_injection_smoke.sh` is available. | Need missing-policy evidence with degraded/bypass and no OVS forwarding disruption. | Run ACL fault smoke with a binding that references a missing policy, then capture status and rollback. |
| S3-3 apply-failure | fault | pending | `deploy/kolla/smoke/neutron_aria_acl_fault_injection_smoke.sh` is available. | Need compile/apply failure evidence with no false ready. | Run one-shot fault points around ACL apply and capture WAL/status/recovery output. |
