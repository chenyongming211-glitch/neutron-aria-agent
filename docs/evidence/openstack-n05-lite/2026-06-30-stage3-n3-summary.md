...
| S3-4 ovs-restart | lifecycle | degraded | `docs/evidence/openstack-n05-lite/20260701-stage3-ovs-restart-probe/summary.md` | Controlled `ovs-vswitchd.service` restart on `ostack2.bj159.net` showed that tap and XDP remained present while immediate VM ping failed during the OVS restart window. This evidence is degraded because the temporary probe mixed ACL attach health with OVS forwarding health. Cleanup deleted all managed ACL ports, UDS returned to `ready` with zero managed ports, and VM connectivity recovered. | Rerun an ACL-focused ovs-restart smoke that separately records tap/ifindex/XDP/map/generation/rollback for Aria and VM ping as OVS forwarding evidence. |
