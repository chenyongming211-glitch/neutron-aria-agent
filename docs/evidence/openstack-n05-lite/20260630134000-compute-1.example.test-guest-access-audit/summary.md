...
| --- | --- | --- | --- |
| `wp-test` | `192.0.2.26` | `server-wp-test.txt`, `ssh-192.0.2.26.txt`, `qga-check.txt` | `key_name=null`; SSH rc=1; QEMU guest agent not configured |
| `test1111` | `192.0.2.27` | `server-test1111.txt`, `ssh-192.0.2.27.txt`, `qga-check.txt` | `key_name=null`; SSH rc=1; console class is `SecOS`; QEMU guest agent not configured |
| `cym_vfw1` | `192.0.2.28` | `server-cym-vfw1.txt`, `ssh-192.0.2.28.txt`, `qga-check.txt` | `key_name=null`; SSH rc=0 but prior root/admin/centos key auth was denied; QEMU guest agent not configured |
| `cym_hlas_test` | `192.0.2.29` | `server-cym-hlas-test.txt`, `ssh-192.0.2.29.txt`, `qga-check.txt` | `key_name=null`; SSH rc=1; console is Rocky Linux `LAS login:` with cloud-init fallback datasource; QEMU guest agent not configured |
