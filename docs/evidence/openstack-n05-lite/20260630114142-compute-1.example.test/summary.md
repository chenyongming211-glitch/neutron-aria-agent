...
Generated at: `2026-06-30T03:41:53Z`
...
| Trunk extension | Record trunk support disposition; unsupported is acceptable for ACL MVP | `check_trunk_extension` | exit=1 | `trunk-extension.txt` | unsupported |
...
| OVS iface-id external_ids | OVS interfaces expose external_ids:iface-id | `check_ovs_iface_id` | exit=0 | `ovs-iface-id.txt` | pass |
...
| XDP tap status | Record current tap XDP status without attaching anything | `collect_xdp_status` | exit=0 | `xdp-status.txt` | pass |
...
| Neutron port class disposition | Record local vnic_type counts and unsupported port classes | `collect_neutron_port_classes` | exit=0 | `port-classes.txt` | pass |
