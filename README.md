...
基于 eBPF/XDP 的高性能防火墙，支持 LPM 前缀树进行 CIDR 匹配的 IP 组和基于端口集 (port set) 的策略规则。
...
- **内存高效**：LPM 前缀树和端口集 (port set) 都是高度优化的数据结构
...
- **端口集 (port set)**：紧凑的端口策略存储，支持引用计数优化
- **状态持久化**：服务重启后自动恢复规则状态
...
2. **端口集 (port set)**：紧凑存储端口策略，支持引用计数共享相同端口集的规则
